...
import { collectWorkerStates, EventBus, MemoryRepository, Task, TaskStatus, createFleetTransferService, createHelpCommandService, createHelpService, splitWeightedWork } from '../src/index.js';
...
class BotAdapter {
  ...
  snapshot() { return { inventorySummary: structuredClone(this.items), inventorySlots: this.capacity ?? 36, inventorySlotsUsed: this.slotsUsed ?? 0, inventorySlotsFree: this.slotsFree ?? (this.capacity ?? 36), freeItemCapacity: this.capacityUnits ?? ((this.capacity ?? 36) * 64), position: this.position, dimension: 'overworld' }; }
  async smartMove() {}
...
function setup(target = 64, options = {}) {
  ...
  const events = new EventBus(); const goals = { task: id => { if (id !== parent.id) throw new Error(`Task '${id}' not found`); return parent; }, allTasks: () => [parent.toDTO()], completeExternalTask: async (id, result) => { if (id !== parent.id) throw new Error(`Task '${id}' not found`); parent.update(TaskStatus.COMPLETED, { result }); return parent.toDTO(); }, transitionTaskToCollaborative: async (id, helpSessionId) => { if (id !== parent.id) throw new Error(`Task '${id}' not found`); parent.update(TaskStatus.COLLABORATIVE, { helpSessionId }); return parent.toDTO(); } }; const executor = options.executorFactory ? options.executorFactory(runtimes) : { cancel: () => true, status: () => ({ running: {}, queues: {} }), execute: async task => { change(runtimes[task.assignedBot].adapter.items, 'stone', task.input.count); task.update(TaskStatus.COMPLETED); return { collected: task.input.count }; } }; const logistics = { store: async ({ runtime: value, item, count }) => { change(value.adapter.items, item, -count); return { stored: count }; }, storages: async () => options.storages ?? [] };
  const repositories = { sessions: new MemoryRepository(), shares: new MemoryRepository(), contributions: new MemoryRepository() }; const service = createHelpService({ repository: repositories.sessions, workShareRepository: repositories.shares, contributionRepository: repositories.contributions, bots: { get: id => runtimes[id], list: () => Object.values(runtimes) }, fleetTransfer: createFleetTransferService({ events }), logistics, goals, executor, events, maxHelpersPerSession: options.maxHelpersPerSession ?? 4, minimumChunk: options.minimumChunk ?? 4, coordination: options.coordination }); return { owner, helper, helper2, helper3, parent, service, repositories, bots: { get: id => runtimes[id], list: () => Object.values(runtimes) }, fleetTransfer: createFleetTransferService({ events }), logistics, goals, executor, events };
}
...
test('different helpers execute shares concurrently without holding the session lock', async () => {
  const started = []; const releases = []; const context = setup(64, { ownerStone: 0, executorFactory: runtimes => ({ cancel: () => true, execute: task => new Promise(resolve => { started.push(task); releases.push(() => { change(runtimes[task.assignedBot].adapter.items, 'stone', task.input.count); resolve({ collected: task.input.count }); }); }) }) }); const session = await context.service.create({ parentTaskId: context.parent.id, ownerBotId: 'owner', workers: ['helper', 'helper2'] }); const running = session.workShares.map(share => context.service.executeShare({ sessionId: session.id, shareId: share.shareId })); await waitFor(() => started.length === 2, 100, 5); assert.deepEqual(started.map(task => task.assignedBot).sort(), ['helper', 'helper2']); await assert.rejects(context.service.rebalanceSession({ sessionId: session.id, reason: 'MANUAL_REBALANCE' }), /HELP_RECONCILIATION_REQUIRED/); releases.forEach(release => release()); await Promise.all(running);
});

test('pause cancels a running helper without waiting for its execution lock', async () => {
  const cancelled = []; let runningTaskId = null; const context = setup(49, { executorFactory: () => { const pending = new Map(); return { cancel: taskId => { cancelled.push(taskId); pending.get(taskId)?.(); return true; }, execute: task => new Promise((resolve, reject) => { runningTaskId = task.id; pending.set(task.id, () => reject(Object.assign(new Error('cancelled'), { code: 'CANCELLED' }))); }) }; } }); const session = await context.service.create({ parentTaskId: context.parent.id, ownerBotId: 'owner', workers: ['helper'] }); const execution = context.service.executeShare({ sessionId: session.id, shareId: session.workShares[0].shareId }); await waitFor(() => Boolean(runningTaskId), 100, 5); const paused = await context.service.pause({ sessionId: session.id, botId: 'helper' }); assert.equal(cancelled.length, 1); assert.equal(paused.workShares[0].status, 'PAUSED'); await assert.rejects(execution, /cancelled/);
});

test('large work shares run in verified 64-item batches until parent completion', async () => {
  const context = setup(162, { ownerStone: 34 }); const session = await context.service.create({ parentTaskId: context.parent.id, ownerBotId: 'owner', workers: ['helper'] }); let current = session; for (let batch = 0; batch < 2; batch++) { current = await context.service.executeShare({ sessionId: current.id, shareId: current.workShares.at(-1).shareId }); assert.equal(current.workShares.at(-1).completed, (batch + 1) * 64); current = await context.service.handoff({ sessionId: current.id, shareId: current.workShares.at(-1).shareId }); } assert.equal(current.status, 'COMPLETED'); assert.equal(context.parent.status, TaskStatus.COMPLETED);
});

test('a 300-item share resumes safely after restart between verified batches', async () => {
  const context = setup(334, { ownerStone: 34 }); let session = await context.service.create({ parentTaskId: context.parent.id, ownerBotId: 'owner', workers: ['helper'] }); session = await context.service.executeShare({ sessionId: session.id, shareId: session.workShares[0].shareId }); session = await context.service.handoff({ sessionId: session.id, shareId: session.workShares[0].shareId }); const restored = createHelpService({ repository: context.repositories.sessions, workShareRepository: context.repositories.shares, contributionRepository: context.repositories.contributions, bots: context.bots, fleetTransfer: context.fleetTransfer, logistics: context.logistics, goals: context.goals, executor: context.executor, events: context.events, maxHelpersPerSession: 4, minimumChunk: 4 }); await restored.initialize(); for (let batch = 0; batch < 4; batch++) { session = await restored.executeShare({ sessionId: session.id, shareId: session.workShares[0].shareId }); session = await restored.handoff({ sessionId: session.id, shareId: session.workShares[0].shareId }); } assert.equal(session.status, 'COMPLETED'); assert.equal(count(context.owner.adapter.items, 'stone'), 334);
});

test('worker capacity uses real slot fields including partial target stacks', () => {
  const context = setup(); context.helper.adapter.slotsUsed = 36; context.helper.adapter.slotsFree = 0; context.helper.adapter.capacityUnits = 0; const full = collectWorkerStates({ session: { id: 'session', goal: { item: 'stone' } }, shares: [{ shareId: 'share', botId: 'helper', role: 'HELPER', status: 'ASSIGNED', assigned: 16, completed: 0, delivered: 0, credited: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }], bots: context.bots, now: Date.now() })[0]; assert.equal(full.inventoryFreeSlots, 0); assert.equal(full.capacityUnits, 0); context.helper.adapter.slotsUsed = 35; context.helper.adapter.slotsFree = 1; context.helper.adapter.capacityUnits = 80; const partial = collectWorkerStates({ session: { id: 'session', goal: { item: 'stone' } }, shares: [{ shareId: 'share', botId: 'helper', role: 'HELPER', status: 'ASSIGNED', assigned: 16, completed: 0, delivered: 0, credited: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }], bots: context.bots, now: Date.now() })[0]; assert.equal(partial.inventoryFreeSlots, 1); assert.equal(partial.capacityUnits, 80);
});

test('stall detection requires no progress beyond the configured threshold and rebalance completion uses GoalService', async () => {
  const context = setup(64, { ownerStone: 0, coordination: { minRebalanceIntervalMs: 1, minimumTransferUnits: 4, minimumBenefitRatio: 0.15, maxRebalancesPerSession: 20, progressStallThresholdMs: 10 } }); const session = await context.service.create({ parentTaskId: context.parent.id, ownerBotId: 'owner', workers: ['helper'] }); const share = session.workShares[0]; await context.repositories.shares.update(share.shareId, { ...share, lastProgressAt: new Date(Date.now() - 20).toISOString() }); assert.equal((await context.service.stalledWorkers(session.id)).length, 1); change(context.owner.adapter.items, 'stone', 64); const complete = await context.service.rebalanceSession({ sessionId: session.id, reason: 'MANUAL_REBALANCE', rebalanceKey: 'complete-parent' }); assert.equal(complete.status, 'COMPLETED'); assert.equal(context.parent.status, TaskStatus.COMPLETED);
});

function runtime(id, stone, x) { return { id, status: 'READY', bot: { id }, options: { host: 'server', port: 25565 }, adapter: new BotAdapter([{ name: 'stone_pickaxe', count: 1 }, ...(stone ? [{ name: 'stone', count: stone }] : [])], { x, y: 64, z: 0 }) }; }

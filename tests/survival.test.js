...
test('manual armor rejects downgrade and auto armor preserves stronger enchantments', async () => {
  const { adapter, client } = await readyAdapter(); client.equipment.head = { name: 'iron_helmet', count: 1, durabilityUsed: 0, enchants: [{ name: 'protection', lvl: 4 }] }; client.items.push({ name: 'leather_helmet', count: 1, durabilityUsed: 0 }, { name: 'diamond_helmet', count: 1, durabilityUsed: 0 });
  await assert.rejects(adapter.equipArmor({ item: 'leather_helmet', preserveDurability: true, minimumDurability: 10, preferProtection: true, preferDurability: false, allowBindingCurse: false }), error => error.code === 'ARMOR_DOWNGRADE_REJECTED'); const result = await adapter.autoEquipArmor({ preserveDurability: true, minimumDurability: 10, preferProtection: true, preferDurability: false, allowBindingCurse: false }); assert.equal(result.changed, false); assert.equal(result.after.equipped.head.enchantmentProtection, 4); await adapter.disconnect();
});
...
test('sleep records failed occupancy and wakes when cancellation arrives during sleep', async () => {
  const { adapter, client } = await readyAdapter(); const bed = fakeBlock('white_bed', new Vec3(2, 64, 0), { occupied: true }); client.blocks.set(positionKey(bed.position), bed); adapter.navigate = async () => ({ position: bed.position });
  await assert.rejects(adapter.sleep({ position: bed.position, maxDistance: 16 }, {}), error => error.code === 'BED_OCCUPIED'); assert.equal(adapter.sleepStatus().state, 'FAILED'); bed.properties.occupied = false; const controller = new AbortController(); client.sleep = async () => { client.isSleeping = true; controller.abort(new Error('sleep cancelled')); await new Promise(resolve => setImmediate(resolve)); };
  await assert.rejects(adapter.sleep({ position: bed.position, maxDistance: 16 }, { signal: controller.signal }), /sleep cancelled/); assert.equal(client.isSleeping, false); assert.equal(adapter.sleepStatus().state, 'FAILED'); await adapter.disconnect();
});

test('door and trapdoor interactions enforce exact types and reject iron variants', async () => {
  const { adapter, client } = await readyAdapter(); const door = fakeBlock('oak_door', new Vec3(1, 64, 0), { open: false }); const trapdoor = fakeBlock('oak_trapdoor', new Vec3(2, 64, 0), { open: true }); const iron = fakeBlock('iron_door', new Vec3(3, 64, 0), { open: false }); const ironTrapdoor = fakeBlock('iron_trapdoor', new Vec3(4, 64, 0), { open: false }); for (const block of [door, trapdoor, iron, ironTrapdoor]) client.blocks.set(positionKey(block.position), block);
  assert.equal((await adapter.openDoor({ position: door.position, cooldownMs: 100 })).after.open, true); assert.equal((await adapter.openDoor({ position: door.position, cooldownMs: 100 })).changed, false); assert.equal((await adapter.closeTrapdoor({ position: trapdoor.position, cooldownMs: 100 })).after.open, false); await assert.rejects(adapter.openDoor({ position: trapdoor.position, cooldownMs: 100 }), error => error.code === 'BLOCK_NOT_FOUND'); await assert.rejects(adapter.openDoor({ position: iron.position, cooldownMs: 100 }), error => error.code === 'CAPABILITY_UNAVAILABLE'); await assert.rejects(adapter.openTrapdoor({ position: ironTrapdoor.position, cooldownMs: 100 }), error => error.code === 'CAPABILITY_UNAVAILABLE'); await adapter.disconnect();
});

test('generic item, entity, and block interactions execute their verified contracts', async () => {
  const { adapter, client } = await readyAdapter(); client.items.push({ name: 'stick', count: 1 }); const cow = { id: 9, type: 'mob', name: 'cow', position: new Vec3(1, 64, 0) }; client.entities = { 9: cow }; const lever = fakeBlock('lever', new Vec3(2, 64, 0), { open: false }); client.blocks.set(positionKey(lever.position), lever);
  assert.equal((await adapter.useItem({ item: 'stick' }, {})).verified, true); assert.equal(client.itemActivated, true); assert.equal(client.itemDeactivated, true); assert.equal((await adapter.interactEntity({ entityId: 9, expectedType: 'cow', action: 'INSPECT' })).entity.entityId, '9'); assert.equal((await adapter.interactBlock({ position: lever.position, action: 'TOGGLE', cooldownMs: 100 })).verified, true); await adapter.disconnect();
});

test('acquisition resolves wool and milk through registered survival special sources', async () => {
  const items = []; const adapter = { snapshot: () => ({ inventorySummary: items, dimension: 'overworld', position: { x: 0, y: 64, z: 0 } }), craftRequirements: async ({ item }) => item === 'white_wool' ? { craftable: true, missing: [{ name: 'string', count: 4 }] } : ({ craftable: ['shears', 'bucket'].includes(item), missing: [] }), craftItem: async ({ item, count }) => { addItem(items, item, count); return { item, count }; }, smeltRequirements: async () => null, findSourceBlocks: async () => [], acquireWool: async ({ count }) => { addItem(items, 'white_wool', count); return { acquired: count }; }, acquireMilk: async ({ count }) => { addItem(items, 'bucket', -count); addItem(items, 'milk_bucket', count); return { acquired: count }; } };

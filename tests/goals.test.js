...
test('reservation failure marks task failed and leaves no executor zombie', async () => {
  const app = createApplication({ env: { MINEHIVE_PROFILE: 'test', MINEHIVE_LOG_LEVEL: 'silent' } }); const failed = []; const reservationFailures = []; app.events.subscribe('task.failed', event => failed.push(event.payload)); app.events.subscribe('resource.reservation.failed', event => reservationFailures.push(event.payload)); app.bots.create({ id: 'worker', capabilities: ['work'] }); app.capabilities.register({ name: 'work', execute: async () => { throw new Error('capability must not run'); } }); const goal = app.goals.create({ description: 'Protected task input', constraints: { preferredBot: 'worker' }, steps: [{ type: 'work', input: {}, resources: { inputs: [{ item: 'stone', count: 1 }], outputs: [] }, requiredCapabilities: ['work'] }] }); const result = await app.goals.run(goal.id); assert.equal(result.status, 'FAILED'); assert.equal(app.goals.tasks(goal.id)[0].status, 'FAILED'); assert.deepEqual(app.executor.status().running, {}); assert.equal(app.executor.status().queuedTasks, 0); assert.equal(failed.length, 1); assert.equal(reservationFailures.length, 1);
});

test('task resource ownership and reason survive planning and release after completion', async () => {
  const app = createApplication({ env: { MINEHIVE_PROFILE: 'test', MINEHIVE_LOG_LEVEL: 'silent' } }); app.bots.create({ id: 'worker', capabilities: ['work'] }); const runtime = app.bots.get('worker'); runtime.adapter.snapshot = () => ({ inventorySummary: [{ name: 'coal', count: 1 }] }); app.capabilities.register({ name: 'work', execute: async () => ({ ok: true }) }); const goal = app.goals.create({ description: 'Acquisition contract', constraints: { preferredBot: 'worker' }, steps: [{ type: 'work', input: {}, resources: { ownerType: 'ACQUISITION', reason: 'ACQUISITION_RESERVED', inputs: [{ item: 'coal', count: 1 }], outputs: [{ item: 'torch', count: 4 }] }, requiredCapabilities: ['work'] }] }); const result = await app.goals.run(goal.id); assert.equal(result.status, 'COMPLETED'); const reservations = app.resourceReservations.reservationsForBot('worker'); assert.equal(reservations.length, 2); assert.ok(reservations.every(entry => entry.ownerType === 'ACQUISITION' && entry.reason === 'ACQUISITION_RESERVED' && entry.status === 'RELEASED'));
});

...
test('API exposes health and versioned bot DTOs', async () => {
  ...
  try {
    ...
    const navigationStatus = await fetch(`http://127.0.0.1:${port}/api/v1/navigation/status`); assert.equal(navigationStatus.status, 200); assert.equal((await navigationStatus.json()).data.active, 0); assert.deepEqual((await (await fetch(`http://127.0.0.1:${port}/api/v1/navigation/status`)).json()).data.activeGroups, []);
    const invalidGroupMove = await fetch(`http://127.0.0.1:${port}/api/v1/navigation/group/move`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"botIds":["api-bot"],"anchor":{"x":0,"y":64,"z":0}}' }); assert.equal(invalidGroupMove.status, 400); assert.equal((await invalidGroupMove.json()).error.code, 'INVALID_GROUP');
    const territoryCreated = await fetch(`http://127.0.0.1:${port}/api/v1/territory/regions`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"worldKey":"localhost:25565","dimension":"overworld","name":"API Base","type":"BASE","center":{"x":0,"y":64,"z":0},"radius":32,"explored":true}' }); const territoryRegion = (await territoryCreated.json()).data; assert.equal(territoryCreated.status, 201); assert.equal(territoryRegion.type, 'BASE'); const territoryMap = await (await fetch(`http://127.0.0.1:${port}/api/v1/territory/map?worldKey=localhost:25565&dimension=overworld`)).json(); assert.equal(territoryMap.data.regionCount, 1);
    const dangerSignal = await fetch(`http://127.0.0.1:${port}/api/v1/territory/signals/danger`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"worldKey":"localhost:25565","dimension":"overworld","position":{"x":100,"y":64,"z":100},"severity":0.8,"confidence":0.9,"source":"API"}' }); assert.equal(dangerSignal.status, 201); assert.equal((await dangerSignal.json()).data.type, 'DANGER'); assert.equal((await (await fetch(`http://127.0.0.1:${port}/api/v1/territory/intelligence`)).json()).data.dangerSignals, 1);
    const seededFrontiers = await fetch(`http://127.0.0.1:${port}/api/v1/territory/frontiers/seed`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"worldKey":"localhost:25565","dimension":"overworld","origin":{"x":0,"y":64,"z":0},"distance":128}' }); assert.equal(seededFrontiers.status, 201); assert.equal((await seededFrontiers.json()).data.length, 8); assert.equal((await (await fetch(`http://127.0.0.1:${port}/api/v1/exploration/status`)).json()).data.missions, 0);
    ...
    const governance = await fetch(`http://127.0.0.1:${port}/api/v1/memory/governance/status`); assert.equal(governance.status, 200); assert.equal((await governance.json()).data.schemaVersion, 1); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/audit`)).status, 200); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/quarantine`)).status, 200); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/archive`)).status, 200);
    const working = await fetch(`http://127.0.0.1:${port}/api/v1/memory/working`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"taskId":"api-working","botId":"api-bot","currentAction":"survey","target":{"radius":32}}' }); assert.equal(working.status, 201); assert.equal((await working.json()).data.taskId, 'api-working'); const workingPatch = await fetch(`http://127.0.0.1:${port}/api/v1/memory/working/api-working`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: '{"progress":{"completed":1,"total":2}}' }); assert.equal(workingPatch.status, 200); assert.equal((await workingPatch.json()).data.progress.ratio, 0.5); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/working?botId=api-bot`)).status, 200); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/working/api-working`, { method: 'DELETE' })).status, 200);
    const episodic = await fetch(`http://127.0.0.1:${port}/api/v1/memory/episodic`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"episodeType":"OBSERVATION","sourceId":"api-episode","botId":"api-bot","action":"survey","outcome":"OBSERVED","lesson":"lava ditemukan dekat jalur timur","position":{"x":20,"y":40,"z":5}}' }); const episodicPayload = (await episodic.json()).data; assert.equal(episodic.status, 201); assert.equal((await (await fetch(`http://127.0.0.1:${port}/api/v1/memory/episodic?q=lava&botId=api-bot`)).json()).data[0].sourceId, 'api-episode'); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/episodic/${episodicPayload.id}`)).status, 200);
    const knowledge = await fetch(`http://127.0.0.1:${port}/api/v1/memory/knowledge`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"knowledgeType":"PROCEDURE","key":"api:survey-safe","name":"Survei aman","intent":"survey","status":"ACTIVE","steps":[{"action":"observe"},{"action":"report"}],"evidence":{"samples":3,"successes":3}}' }); const knowledgePayload = (await knowledge.json()).data; assert.equal(knowledge.status, 201); assert.equal(knowledgePayload.version, 1); assert.equal((await (await fetch(`http://127.0.0.1:${port}/api/v1/memory/knowledge?q=survei&knowledgeType=PROCEDURE&status=ACTIVE`)).json()).data[0].key, 'api:survey-safe');
    const revisedKnowledge = await fetch(`http://127.0.0.1:${port}/api/v1/memory/knowledge/${knowledgePayload.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: '{"expectedVersion":1,"name":"Survei sangat aman"}' }); assert.equal(revisedKnowledge.status, 200); assert.equal((await revisedKnowledge.json()).data.version, 2); const knowledgeFeedback = await fetch(`http://127.0.0.1:${port}/api/v1/memory/knowledge/${knowledgePayload.id}/feedback`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{"success":true,"episodeId":"api-episode"}' }); assert.equal(knowledgeFeedback.status, 200); assert.equal((await knowledgeFeedback.json()).data.evidence.samples, 4); assert.equal((await fetch(`http://127.0.0.1:${port}/api/v1/memory/knowledge/${knowledgePayload.id}`)).status, 200);

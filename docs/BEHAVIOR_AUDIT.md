...
| Command/query buses | Fixed | `Application` registers command handlers for `goal.create`, `goal.run`, and `goal.cancel`, plus query handlers for `goal.get`, `goal.list`, and `goal.tasks`. | No action; update this audit if the registered handler set changes. |

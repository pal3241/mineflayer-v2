...
`bot1` menjadi pemimpin. Command ini boleh dimulai dengan inventory kosong: bot lebih dulu mencari tanaman matang; jika tidak tersedia, bot hanya mengambil hewan surplus dan selalu menyisakan minimal dua hewan per jenis. Hanya bot READY pada server, dimension, dan grup yang sama yang masuk squad serta tetap dalam formasi 10–15 blok; pengumpulan dihentikan sementara bila ada anggota yang tertinggal.

Urutan progres adalah kayu → batu → rumah/storehouse/farm murah → persiapan gua → iron → bedroom/workshop → verifikasi farm gandum dan area tanam pohon. Blueprint bawaan diberi kategori `HOUSE`, `STOREHOUSE`, `CROP_FARM`, `TREE_FARM`, dan `WORKSHOP`. Pemilih otomatis mendahulukan nilai early-game tertinggi, lalu mengurangi skor untuk material yang belum dimiliki dan komponen yang lebih merepotkan. Chest, pintu, fence gate, composter, furnace, dan komponen lain dibuat lewat acquisition/crafting sebelum build.

Sebelum ekspedisi iron, pemimpin wajib membawa minimal 16 makanan, senjata, dan 16 obor. Iron dicari melalui ore terbuka di gua dengan combat guard aktif; mode early-game tidak beralih ke strip-mine jika gua aman belum tersedia. Early game baru memasuki mode `SELF_SUFFICIENT` setelah pemimpin memiliki iron chestplate, shield, dan iron pickaxe serta sumber makanan dan kayu terbarukan sudah terverifikasi.

`SELF_SUFFICIENT` bukan akhir scheduler. MineHive terus memelihara cadangan makanan, kayu, obor, batu bara, sapling, equipment, menjalankan siklus farm, menanam kembali pohon, dan mendaftarkan home. Tahap teknologi disimpan secara monoton sehingga tidak kembali ke WOOD hanya karena material dipakai untuk bangunan. Tiga kegagalan berturut-turut membuat health early-game menjadi `DEGRADED`, bukan diam-diam mengaku sehat. Saat command early-game aktif, reconnect anggota squad dibuat tidak terbatas; runtime biasa tetap memakai batas `MINEHIVE_RECONNECT_ATTEMPTS` agar proses lain dapat berhenti bersih.

Blueprint otomatis hanya dibangun setelah makanan dan material cukup serta scanner menemukan permukaan yang luas, terang, relatif rata, tanpa hostile atau hazard. Hanya blueprint bawaan tepercaya yang boleh melewati approval, bukan file pengguna. Gunakan `!global early_game status|pause|resume|stop` untuk mengelola siklusnya.

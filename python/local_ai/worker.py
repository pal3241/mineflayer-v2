        ...
        try:
            ...
            if action=='initialize':
                ...
                if checkpoint.exists():
                    ...
                    else:result=train_model(texts,checkpoint,request.get('epochs',12));model,metadata=result['model'],result['payload']
                else:result=train_model(texts,checkpoint,request.get('epochs',12));model,metadata=result['model'],result['payload']
                respond(request,status(model,metadata,checkpoint))
            elif action=='train':
                result=train_model(request.get('texts',[]),checkpoint,request.get('epochs',12));model,metadata=result['model'],result['payload'];respond(request,status(model,metadata,checkpoint))

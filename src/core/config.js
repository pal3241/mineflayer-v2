...
export function loadConfig(env = process.env) {
  ...
  const nvidiaEndpoint = env.MINEHIVE_NVIDIA_NIM_ENDPOINT ?? env.MINEHIVE_LOCAL_LLM_ENDPOINT ?? 'https://integrate.api.nvidia.com/v1'; const nvidiaModel = env.MINEHIVE_NVIDIA_NIM_MODEL ?? env.MINEHIVE_LOCAL_LLM_MODEL ?? 'nvidia/nemotron-3-nano-30b-a3b'; const nvidiaConfigured = nvidiaKeys.length > 0 || Boolean((env.MINEHIVE_NVIDIA_NIM_ENDPOINT ?? env.MINEHIVE_LOCAL_LLM_ENDPOINT) && (env.MINEHIVE_NVIDIA_NIM_MODEL ?? env.MINEHIVE_LOCAL_LLM_MODEL));
  const config = {
    profile,
    ...
    semanticMemory: { maxRecords: memoryMaxRecords, longTermMaxRecords: integer(env.MINEHIVE_LONG_MEMORY_MAX_RECORDS, memoryMaxRecords), workingMemoryMaxRecords: integer(env.MINEHIVE_WORKING_MEMORY_MAX_RECORDS, 1000), workingMemoryTtlMs: integer(env.MINEHIVE_WORKING_MEMORY_TTL_MS, 1_800_000), episodicMemoryMaxRecords: integer(env.MINEHIVE_EPISODIC_MEMORY_MAX_RECORDS, memoryMaxRecords), knowledgeMaxRecords: integer(env.MINEHIVE_KNOWLEDGE_MAX_RECORDS, 5000), knowledgeMinimumEvidence: integer(env.MINEHIVE_KNOWLEDGE_MINIMUM_EVIDENCE, 3), eventStreamMaxRecords: integer(env.MINEHIVE_MEMORY_EVENT_STREAM_MAX_RECORDS, 50_000), dimensions: integer(env.MINEHIVE_EMBEDDING_DIMENSIONS, 256), shortTermMaxRecords: integer(env.MINEHIVE_SHORT_MEMORY_MAX_RECORDS, Math.min(1000, memoryMaxRecords)), shortTermTtlMs: integer(env.MINEHIVE_SHORT_MEMORY_TTL_MS, 86_400_000), promotionAccesses: integer(env.MINEHIVE_MEMORY_PROMOTION_ACCESSES, 3), promotionImportance: Number(env.MINEHIVE_MEMORY_PROMOTION_IMPORTANCE ?? 0.8), consolidationIntervalMs: integer(env.MINEHIVE_MEMORY_CONSOLIDATION_INTERVAL_MS, 60_000) },
  ...
  if (config.semanticMemory.shortTermMaxRecords < 1 || config.semanticMemory.shortTermMaxRecords > config.semanticMemory.maxRecords || config.semanticMemory.longTermMaxRecords < 1 || config.semanticMemory.longTermMaxRecords > config.semanticMemory.maxRecords || config.semanticMemory.workingMemoryMaxRecords < 1 || config.semanticMemory.workingMemoryTtlMs < 1000 || config.semanticMemory.episodicMemoryMaxRecords < 1 || config.semanticMemory.knowledgeMaxRecords < 1 || config.semanticMemory.knowledgeMinimumEvidence < 2 || config.semanticMemory.eventStreamMaxRecords < 100 || config.semanticMemory.shortTermTtlMs < 1000 || config.semanticMemory.promotionAccesses < 1 || !Number.isFinite(config.semanticMemory.promotionImportance) || config.semanticMemory.promotionImportance < 0 || config.semanticMemory.promotionImportance > 1 || config.semanticMemory.consolidationIntervalMs < 5000) throw new ValidationError('Invalid short-term memory lifecycle policy');
...
function resolveLlmProvider(value, hasOpenRouter, hasNvidia) { const requested = value ?? 'auto'; if (requested === 'auto') return hasNvidia ? 'nvidia' : hasOpenRouter ? 'openrouter' : 'none'; if (requested === 'local') return 'nvidia'; return requested; }

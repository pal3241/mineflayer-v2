...
export function loadConfig(env = process.env) {
  ...
  const config = {
    ...
    semanticMemory: { maxRecords: memoryMaxRecords, longTermMaxRecords: integer(env.MINEHIVE_LONG_MEMORY_MAX_RECORDS, memoryMaxRecords), workingMemoryMaxRecords: integer(env.MINEHIVE_WORKING_MEMORY_MAX_RECORDS, 1000), workingMemoryTtlMs: integer(env.MINEHIVE_WORKING_MEMORY_TTL_MS, 1_800_000), episodicMemoryMaxRecords: integer(env.MINEHIVE_EPISODIC_MEMORY_MAX_RECORDS, memoryMaxRecords), knowledgeMaxRecords: integer(env.MINEHIVE_KNOWLEDGE_MAX_RECORDS, 5000), knowledgeMinimumEvidence: integer(env.MINEHIVE_KNOWLEDGE_MINIMUM_EVIDENCE, 3), dimensions: integer(env.MINEHIVE_EMBEDDING_DIMENSIONS, 256), shortTermMaxRecords: integer(env.MINEHIVE_SHORT_MEMORY_MAX_RECORDS, Math.min(1000, memoryMaxRecords)), shortTermTtlMs: integer(env.MINEHIVE_SHORT_MEMORY_TTL_MS, 86_400_000), promotionAccesses: integer(env.MINEHIVE_MEMORY_PROMOTION_ACCESSES, 3), promotionImportance: Number(env.MINEHIVE_MEMORY_PROMOTION_IMPORTANCE ?? 0.8), consolidationIntervalMs: integer(env.MINEHIVE_MEMORY_CONSOLIDATION_INTERVAL_MS, 60_000) },

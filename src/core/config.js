...
export function loadConfig(env = process.env) {
  ...
  if (config.semanticMemory.shortTermMaxRecords < 1 || config.semanticMemory.shortTermMaxRecords > config.semanticMemory.maxRecords || config.semanticMemory.longTermMaxRecords < 1 || config.semanticMemory.longTermMaxRecords > config.semanticMemory.maxRecords || config.semanticMemory.shortTermTtlMs < 1000 || config.semanticMemory.promotionAccesses < 1 || !Number.isFinite(config.semanticMemory.promotionImportance) || config.semanticMemory.promotionImportance < 0 || config.semanticMemory.promotionImportance > 1 || config.semanticMemory.consolidationIntervalMs < 5000) throw new ValidationError('Invalid short-term memory lifecycle policy');

...
export class Application {
  constructor(config, overrides = {}) {
    ...
    this.runtimeDefaults = structuredClone({ llm: config.llm ?? { provider: 'none' }, logLevel: config.log.level, autonomy: config.autonomy, recovery: config.recovery, memory: defaultMemorySettings(config.semanticMemory) });
  ...
  status() { return { name: 'MineHive', version: '0.7.1', state: this.state, uptimeSeconds: this.startedAt ? Math.floor((Date.now() - this.startedAt) / 1000) : 0, bots: this.bots.list(), goals: this.goals.list(), modules: this.modules.status(), plugins: this.plugins.status(), autonomy: this.autonomy.status() }; }
}

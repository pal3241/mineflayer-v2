...
export class Application {
  constructor(config, overrides = {}) {
    ...
    this.building = createBuildingService({ repository: repository('building-projects'), settingsRepository: repository('building-settings'), events: this.events, bots: this.bots, capabilities: this.capabilities, settings: config.building });
  ...
  status() { return { name: 'MineHive', version: '0.9.1', phase: 'Blueprint & Cooperative Construction', state: this.state, uptimeSeconds: this.startedAt ? Math.floor((Date.now() - this.startedAt) / 1000) : 0, bots: this.bots.list(), goals: this.goals.list(), modules: this.modules.status(), plugins: this.plugins.status(), navigation: this.navigation.status(), autonomy: this.autonomy.status(), acquisition: this.acquisition.status(), survival: this.survival.status() }; }
}

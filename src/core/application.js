...
import { createEpisodicMemoryService } from '../memory/episodic-memory-service.js';
import { createKnowledgeService } from '../memory/knowledge-service.js';
import { createMemoryEventStream } from '../memory/memory-event-stream.js';
import { createMemoryIntegrityService } from '../memory/memory-integrity-service.js';
import { createUniversalTaskMemoryService } from '../memory/universal-task-memory-service.js';
...
import { createEarlyGameService } from '../autonomy/early-game-service.js';
...
import { createWorkshopService } from '../logistics/workshop-service.js';
...
import { createAutoHelpCoordinator, createCoordinationMonitor, createHelpCommandService, createHelpService } from '../help/index.js';
...
import { createBuildingService } from '../building/index.js';
import { createClientBridgeService } from '../client/client-bridge-service.js';
import { createCombatService } from '../combat/index.js';
...
export class Application {
  constructor(config, overrides = {}) {
    ...
    // Keep the buses useful for integrations (API, plugins and external clients)
    // instead of constructing empty infrastructure that can never be reached.
    this.commands.register('goal.create', (payload) => this.goals.create(payload));
    this.commands.register('goal.run', (payload) => this.goals.run(payload.id));
    this.commands.register('goal.cancel', (payload) => this.goals.cancel(payload.id, payload.reason));
    this.queries.register('goal.get', (payload) => this.goals.get(payload.id).toDTO());
    this.queries.register('goal.list', () => this.goals.list());
    this.queries.register('goal.tasks', (payload) => this.goals.tasks(payload.id));
    ...
    this.memoryEventStream = createMemoryEventStream({ repository: repository('memory-event-stream'), checkpointRepository: repository('memory-event-checkpoints'), events: this.events, maxRecords: config.semanticMemory.eventStreamMaxRecords ?? 50_000 });
    this.memoryIntegrity = createMemoryIntegrityService({ repositories: { candidates: repository('memory-integrity-candidates'), decisions: repository('memory-integrity-decisions'), recovery: repository('memory-integrity-recovery') }, events: this.events, governance: this.memoryGovernance, minimumConfidence: config.semanticMemory.integrityMinimumConfidence ?? 0.7, minimumConfirmations: config.semanticMemory.integrityMinimumConfirmations ?? 2, maxRecoveryAttempts: config.semanticMemory.integrityMaxRecoveryAttempts ?? 3 });
    this.universalMemory = createUniversalTaskMemoryService({ repository: repository('universal-task-memory'), events: this.events });
    ...
    this.episodicMemory = createEpisodicMemoryService({ repository: repository('episodic-memory'), events: this.events, governance: this.memoryGovernance, embeddingProvider, maxRecords: config.semanticMemory.episodicMemoryMaxRecords ?? config.semanticMemory.maxRecords });
    this.knowledge = createKnowledgeService({ repository: repository('memory-knowledge'), episodicMemory: this.episodicMemory, embeddingProvider, events: this.events, governance: this.memoryGovernance, maxRecords: config.semanticMemory.knowledgeMaxRecords ?? 5000, minimumEvidence: config.semanticMemory.knowledgeMinimumEvidence ?? 3 });
    ...
    this.workshops = createWorkshopService({ events: this.events, memory: this.universalMemory });
    this.fleetTransfer = createFleetTransferService({ events: this.events, bots: this.bots, repository: repository('fleet-transfer-reliability') });
    this.acquisition = createAcquisitionService({ bots: this.bots, logistics: this.logistics, workshops: this.workshops, events: this.events, logger: this.logger, repository: repository('acquisition-requests'), fleetTransfer: this.fleetTransfer, config: config.acquisition ?? { enabled: true, maxDepth: 8, maxSubtasks: 32, maxAttempts: 3, maxDistance: 2000, storageFirst: true, allowFleet: true, allowCraft: true, allowSmelt: true, allowCollect: true, allowPartial: false, toolPreservation: true } });
    ...
    this.helpCommands = createHelpCommandService({ help: this.help, goals: this.goals, bots: this.bots, events: this.events, fleetTransfer: this.fleetTransfer, maxHelpersPerSession: 4, minimumChunk: 4 });
    ...
    this.survival = createSurvivalService({ acquisition: this.acquisition, events: this.events, logger: this.logger, config: config.survival ?? defaultSurvivalSettings(), bots: this.bots, taskControl: this.executor });
    this.building = createBuildingService({ repository: repository('building-projects'), settingsRepository: repository('building-settings'), events: this.events, bots: this.bots, capabilities: this.capabilities, memory: this.universalMemory, acquisition: this.acquisition, logistics: this.logistics, fleetTransfer: this.fleetTransfer, workshops: this.workshops, settings: config.building });
    this.earlyGame = createEarlyGameService({ repository: repository('early-game-state'), bots: this.bots, capabilities: this.capabilities, acquisition: this.acquisition, building: this.building, environmentModel: this.environmentModel, events: this.events, logger: this.logger, config: config.earlyGame });
    this.autoHelp = createAutoHelpCoordinator({ help: this.help, goals: this.goals, bots: this.bots, events: this.events, earlyGame: this.earlyGame, fleetTransfer: this.fleetTransfer, logger: this.logger, config: config.autoHelp });
    this.llm = new LlmGateway(config.llm ?? { provider: 'none' }, this.logger, this.localBrain); this.coordinator = new FleetCoordinator({ gateway: this.llm, bots: this.bots, goals: this.goals, memory: this.worldMemory, semanticMemory: this.semanticMemory, workingMemory: this.workingMemory, episodicMemory: this.episodicMemory, knowledge: this.knowledge, memoryIntegrity: this.memoryIntegrity, discovery: this.discovery, logistics: this.logistics, workshops: this.workshops, environmentModel: this.environmentModel, acquisition: this.acquisition, ml: this.ml, hive: this.hive, events: this.events, logger: this.logger, maxQueuePerBot: config.tasks?.maxQueuePerBot ?? 100, navigationSettings: this.navigationSettings });
    this.acquisition.configureTaskRunner(async ({ runtime, capability, input, resources }) => { const goal = this.goals.create({ description: `Acquisition ${capability} for ${runtime.id}`, priority: input.priority ?? 70, constraints: { preferredBot: runtime.id }, steps: [{ type: 'acquisition', input, resources, requiredCapabilities: [capability], timeout: 300_000, retries: 1, reportLifecycle: false }] }); await this.goals.run(goal.id); let task = this.goals.tasks(goal.id)[0]; if (task.status === 'COLLABORATIVE') task = await waitForCollaborativeTask(this.goals, task.id, 900_000); if (task.status !== 'COMPLETED') throw new ValidationError(task.error?.message ?? `Acquisition task '${capability}' failed`); return task.result; });
    ...
    this.combat = createCombatService({ repositories: { profiles: repository('combat-profiles'), events: repository('combat-events'), policies: repository('combat-policies'), doctrines: repository('combat-doctrines') }, events: this.events, bots: this.bots, ml: this.ml, threats: this.threats, logger: this.logger });
    ...
    this.chatCommands = new ChatCommandController({ goalService: this.goals, executor: this.executor, capabilities: this.capabilities, coordinator: this.coordinator, helpCommands: this.helpCommands, navigation: this.navigation, botProfiles: this.botProfiles, survival: this.survival, earlyGame: this.earlyGame, config: config.commands ?? { enabled: false, admins: [] }, logger: this.logger });
    ...
    this.events.subscribe('bot.ready', event => this.workshops.scan({ runtime: this.bots.get(event.payload.botId), radius: 48 }).catch(error => this.logger.warn('workshop.auto-scan.failed', { botId: event.payload.botId, error: error.message })));
    ...
    this.bots.onCreated(runtime => { this.chatCommands.attach(runtime); this.structureObserver.attach(runtime); this.environmentObserver.attach(runtime); this.survival.attach(runtime); void this.combat.bind(runtime).catch(error => this.logger.warn('combat.bind.failed', { botId: runtime.bot.id, error: error.message })); });
    this.api = new ApiServer({ application: this, ...config.api, logger: this.logger });
    Object.entries({ config, logger: this.logger, logStore: this.logStore, eventBus: this.events, commands: this.commands, queries: this.queries, health: this.health, metrics: this.metrics, database: this.database, bots: this.bots, capabilities: this.capabilities, goals: this.goals, scheduler: this.scheduler, checkpoints: this.checkpoints, admins: this.admins, botProfiles: this.botProfiles, worldMemory: this.worldMemory, universalMemory: this.universalMemory, semanticMemory: this.semanticMemory, memoryLifecycle: this.memoryLifecycle, discovery: this.discovery, structureObserver: this.structureObserver, logistics: this.logistics, workshops: this.workshops, fleetTransfer: this.fleetTransfer, acquisition: this.acquisition, help: this.help, helpCommands: this.helpCommands, autoHelp: this.autoHelp, coordination: this.coordination, navigation: this.navigation, territory: this.territory, territoryIntelligence: this.territoryIntelligence, territoryLogistics: this.territoryLogistics, threats: this.threats, exploration: this.exploration, expansion: this.expansion, resilience: this.resilience, resourceReservations: this.resourceReservations, resourceReservationCoordinator: this.resourceReservationCoordinator, recovery: this.recovery, survival: this.survival, building: this.building, earlyGame: this.earlyGame, clientBridge: this.clientBridge, combat: this.combat, ml: this.ml, hive: this.hive, autonomy: this.autonomy, llm: this.llm, coordinator: this.coordinator, taskReporter: this.taskReporter }).filter(([, value]) => value !== null).forEach(([name, value]) => this.container.register(name, value));
    this.container.register('workingMemory', this.workingMemory); this.container.register('episodicMemory', this.episodicMemory); this.container.register('knowledge', this.knowledge); this.container.register('memoryGovernance', this.memoryGovernance); this.container.register('memoryEventStream', this.memoryEventStream); this.container.register('memoryIntegrity', this.memoryIntegrity); this.container.register('localBrain', this.localBrain); this.container.register('environmentModel', this.environmentModel);
    ...
    this.health.register('episodicMemory', async () => this.episodicMemory.status());
    this.health.register('knowledge', async () => this.knowledge.status());
    this.health.register('memoryEventStream', async () => this.memoryEventStream.status());
    this.health.register('memoryIntegrity', async () => this.memoryIntegrity.status());
    this.health.register('universalMemory', async () => this.universalMemory.status());
    this.health.register('memoryLifecycle', async () => this.memoryLifecycle.status());
    this.health.register('ml', async () => this.ml.status());
    this.health.register('localCommandBrain', async () => this.localBrain.status());
    this.health.register('environmentModel', async () => this.environmentModel.status());
  ...
  async initialize() {
    ...
    await this.memoryEventStream.initialize(); await this.memoryGovernance.initialize(); await this.memoryIntegrity.initialize(); await this.universalMemory.initialize(); await this.localBrain.initialize(); await this.ml.initialize(); await this.environmentModel.initialize(); await this.workingMemory.initialize(); await this.episodicMemory.initialize(); await this.knowledge.initialize(); await this.admins.initialize(); this.restoredProfiles = await this.botProfiles.initialize(); await this.navigationSettings.initialize(); await this.acquisition.initialize(); await this.help.initialize(); await this.exploration.initialize(); await this.building.initialize(); await this.earlyGame.initialize();
  ...
  async start({ api = true } = {}) {
    ...
    if (api) await this.api.start(); this.state = 'RUNNING'; this.startedAt = Date.now(); await this.memoryLifecycle.tick(); this.memoryLifecycle.start(); this.autonomy.start(); this.earlyGame.start(); this.coordination.start();
    try {
      ...
      this.autonomy.stop(); this.earlyGame.stop(); this.memoryLifecycle.stop(); this.coordination.stop(); await this.api.stop(); this.startedAt = null; this.state = 'READY'; throw error;
    }
  ...
  async memorySettings() { const [semantic, working, episodic, knowledge, eventStream, integrity] = await Promise.all([this.semanticMemory.status(), this.workingMemory.status(), this.episodicMemory.status(), this.knowledge.status(), this.memoryEventStream.status(), this.memoryIntegrity.status()]); return { maxRecords: semantic.maxRecords, ...semantic.policy, workingMemoryMaxRecords: working.maxRecords, workingMemoryTtlMs: working.ttlMs, episodicMemoryMaxRecords: episodic.maxRecords, knowledgeMaxRecords: knowledge.maxRecords, knowledgeMinimumEvidence: knowledge.minimumEvidence, eventStreamMaxRecords: eventStream.maxRecords, integrityMinimumConfidence: integrity.policy.minimumConfidence, integrityMinimumConfirmations: integrity.policy.minimumConfirmations, integrityMaxRecoveryAttempts: integrity.policy.maxRecoveryAttempts, consolidationIntervalMs: this.memoryLifecycle.status().intervalMs, embedding: semantic.embedding }; }
  ...
  status() { return { name: 'MineHive', version: '1.0.0', phase: 'Hivemind Client Control', state: this.state, uptimeSeconds: this.startedAt ? Math.floor((Date.now() - this.startedAt) / 1000) : 0, bots: this.bots.list(), goals: this.goals.list(), modules: this.modules.status(), plugins: this.plugins.status(), navigation: this.navigation.status(), autonomy: this.autonomy.status(), acquisition: this.acquisition.status(), survival: this.survival.status() }; }
}
...
async function waitForCollaborativeTask(goals, taskId, timeoutMs) { const started = Date.now(); while (Date.now() - started < timeoutMs) { const task = goals.task(taskId).toDTO(); if (['COMPLETED', 'FAILED', 'CANCELLED'].includes(task.status)) return task; await new Promise(resolveWait => setTimeout(resolveWait, 50)); } throw new ValidationError(`Collaborative task '${taskId}' exceeded ${timeoutMs}ms`); }

function defaultMemorySettings(config) {
  const maxRecords = config.maxRecords; return { maxRecords, longTermMaxRecords: config.longTermMaxRecords ?? maxRecords, workingMemoryMaxRecords: config.workingMemoryMaxRecords ?? 1000, workingMemoryTtlMs: config.workingMemoryTtlMs ?? 1_800_000, episodicMemoryMaxRecords: config.episodicMemoryMaxRecords ?? maxRecords, knowledgeMaxRecords: config.knowledgeMaxRecords ?? 5000, knowledgeMinimumEvidence: config.knowledgeMinimumEvidence ?? 3, eventStreamMaxRecords: config.eventStreamMaxRecords ?? 50_000, shortTermMaxRecords: config.shortTermMaxRecords ?? Math.min(1000, maxRecords), shortTermTtlMs: config.shortTermTtlMs ?? 86_400_000, promotionAccesses: config.promotionAccesses ?? 3, promotionImportance: config.promotionImportance ?? 0.8, consolidationIntervalMs: config.consolidationIntervalMs ?? 60_000 };
}

function defaultSurvivalSettings() { return { enabled:true,supervisorEnabled:true,supervisorIntervalMs:1000,criticalHealth:8,panicHealth:6,eatAtFood:16,minimumFoodItems:4,autoSleep:true,autoTorch:true,torchLightThreshold:7,torchIntervalMs:15000,emergencyShelter:true,emergencyPillarHeight:3,autoEquipArmor:true,minimumDurabilityPercent:10,preferProtection:true,preferDurability:false,allowBindingCurse:false,allowAnimalKill:false,minimumSheepReserve:2,minimumCowReserve:2,interactionCooldownMs:500,entitySearchDistance:48 }; }

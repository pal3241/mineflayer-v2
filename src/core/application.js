...
export class Application {
  constructor(config, overrides = {}) {
    ...
    this.container.register('workingMemory', this.workingMemory); this.container.register('episodicMemory', this.episodicMemory); this.container.register('memoryGovernance', this.memoryGovernance);
  ...
  async initialize() {
    ...
    await this.memoryGovernance.initialize(); await this.workingMemory.initialize(); await this.episodicMemory.initialize(); await this.admins.initialize(); this.restoredProfiles = await this.botProfiles.initialize(); await this.navigationSettings.initialize(); await this.acquisition.initialize(); await this.help.initialize(); await this.exploration.initialize();
  ...
  async memorySettings() { const [semantic, working, episodic] = await Promise.all([this.semanticMemory.status(), this.workingMemory.status(), this.episodicMemory.status()]); return { maxRecords: semantic.maxRecords, ...semantic.policy, workingMemoryMaxRecords: working.maxRecords, workingMemoryTtlMs: working.ttlMs, episodicMemoryMaxRecords: episodic.maxRecords, consolidationIntervalMs: this.memoryLifecycle.status().intervalMs, embedding: semantic.embedding }; }
  ...
  status() { return { name: 'MineHive', version: '0.9.0', phase: 'Episodic Memory Phase 3', state: this.state, uptimeSeconds: this.startedAt ? Math.floor((Date.now() - this.startedAt) / 1000) : 0, bots: this.bots.list(), goals: this.goals.list(), modules: this.modules.status(), plugins: this.plugins.status(), navigation: this.navigation.status(), autonomy: this.autonomy.status(), acquisition: this.acquisition.status(), survival: this.survival.status() }; }
}

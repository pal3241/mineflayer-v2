...
export class Application {
  constructor(config, overrides = {}) {
    ...
    this.container.register('workingMemory', this.workingMemory); this.container.register('episodicMemory', this.episodicMemory); this.container.register('knowledge', this.knowledge); this.container.register('memoryGovernance', this.memoryGovernance); this.container.register('memoryEventStream', this.memoryEventStream);
  ...
  async memorySettings() { const [semantic, working, episodic, knowledge, eventStream] = await Promise.all([this.semanticMemory.status(), this.workingMemory.status(), this.episodicMemory.status(), this.knowledge.status(), this.memoryEventStream.status()]); return { maxRecords: semantic.maxRecords, ...semantic.policy, workingMemoryMaxRecords: working.maxRecords, workingMemoryTtlMs: working.ttlMs, episodicMemoryMaxRecords: episodic.maxRecords, knowledgeMaxRecords: knowledge.maxRecords, knowledgeMinimumEvidence: knowledge.minimumEvidence, eventStreamMaxRecords: eventStream.maxRecords, consolidationIntervalMs: this.memoryLifecycle.status().intervalMs, embedding: semantic.embedding }; }

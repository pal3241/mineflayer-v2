...
export class SqliteRepository {
  ...
  async update(id, patch) { const current = await this.find(id); const item = structuredClone({ ...current, ...patch, id }); const result = this.connection.prepare('UPDATE records SET data = ?, version = version + 1, updated_at = ? WHERE domain = ? AND id = ?').run(JSON.stringify(item), new Date().toISOString(), this.domain, id); if (!result.changes) throw new NotFoundError('Record', id); return item; }

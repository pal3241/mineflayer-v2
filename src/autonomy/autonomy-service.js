...
const SAFE_ACTIONS = new Set(['collect', 'farm', 'reforest', 'deforest', 'guard', 'status']);
...
export function createAutonomyService({ repository, coordinator, hive, bots, health, events, logger, enabled, intervalMs, maxActionsPerHour }) {
  ...
  const tick = async () => {
    ...
    const cutoff = Date.now() - 3_600_000; while (executions.length && executions[0] < cutoff) executions.shift(); if (executions.length >= maxActionsPerHour) return { status: 'PAUSED', reason: 'hourly action budget exhausted' };
    ...
    const objective = objectives[0]; const owner = `autonomy:${process.pid}`; const lock = await hive.acquireLock({ key: `objective:${objective.id}`, owner, ttlMs: Math.max(30_000, intervalMs) }); if (!lock) return { status: 'LOCKED', objectiveId: objective.id };
  ...
  const start = () => { if (timer) return; timer = setInterval(() => tick().catch(error => logger.error('autonomy.tick.failed', { error: error.message })), intervalMs); timer.unref?.(); };
  const stop = () => { if (timer) clearInterval(timer); timer = null; };
  const setEnabled = value => { active = Boolean(value); return status(); };
  const status = () => ({ status: active ? 'ACTIVE' : 'DISABLED', intervalMs, maxActionsPerHour, actionsLastHour: executions.length, safeActions: [...SAFE_ACTIONS] });
  return Object.freeze({ createObjective, objectives: () => repository.list(), removeObjective: id => repository.delete(id), tick, start, stop, setEnabled, status });
}

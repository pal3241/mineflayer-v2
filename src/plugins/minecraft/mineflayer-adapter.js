...
export class MineflayerAdapter extends EventEmitter {
  ...
  async useItem(input, { signal } = {}) {
    ...
    await this.equipItem({ item, destination: 'hand' }); const before = inventoryCount(bot, item); bot.activateItem?.(); await delay(100); bot.deactivateItem?.(); const held = equipmentView(bot, 'hand');
  ...
  async activateEntity(input) { const bot = this.#ready('activate-entity'); const entity = requiredEntity(bot, input?.entityId, input?.entityType, 'minecraft.interact-entity'); await bot.activateEntity(entity); return { entity: entityView(entity, bot.entity.position), activated: true, verified: true }; }
  ...
  async interactBlock(input) {
    ...
    if (!['ACTIVATE', 'USE', 'TOGGLE', 'OPEN', 'CLOSE'].includes(action)) throw new ValidationError(`Unsupported block interaction action '${action}'`); enforceInteractionCooldown(this.interactionCooldowns, block, input.cooldownMs ?? 500); await bot.activateBlock(block); await delay(100); const afterBlock = bot.blockAt(await blockVector(position)); const after = blockState(afterBlock);
  ...
  async equipArmor(input) { const bot = this.#ready('armor-equip'); const itemName = validItemName(input?.item, 'Armor item'); const destination = armorDestination(itemName); if (!destination) throw new ValidationError(`Item '${itemName}' is not wearable armor`); const before = equipmentView(bot, destination); const result = await this.equipItem({ item: itemName, destination }); return { ...result, slot: destination, upgraded: armorScore(bot, result.after) >= armorScore(bot, before) }; }
  ...
  async sleep(input, { signal } = {}) { const bot = this.#ready('sleep'); if (['the_nether', 'the_end', 'nether', 'end'].includes(String(bot.game?.dimension).toLowerCase())) throw survivalError('SLEEP_UNAVAILABLE', `Sleep is unavailable in dimension '${bot.game?.dimension}'`, 'minecraft.sleep', { dimension: bot.game?.dimension }); if (!sleepTime(bot)) throw survivalError('SLEEP_UNAVAILABLE', 'Sleep is unavailable at the current time', 'minecraft.sleep', { timeOfDay: bot.time?.timeOfDay }); const bed = input?.position ? { position: validRecoveryPosition(input.position), block: null } : this.findBed({ maxDistance: input?.maxDistance ?? 32 }); this.currentSleepState = { state: 'MOVING_TO_BED', bed: bed.position, error: null }; await this.navigate({ ...bed.position, range: 2 }, { signal }); if (signal?.aborted) throw signal.reason ?? survivalError('INTERACTION_TIMEOUT', 'Sleep was cancelled', 'minecraft.sleep', { position: bed.position }); const block = bot.blockAt(await blockVector(bed.position)); if (!block?.name?.endsWith('_bed')) throw survivalError('BLOCK_NOT_FOUND', `Bed disappeared at ${formatPosition(bed.position)}`, 'minecraft.sleep', { position: bed.position }); if (blockState(block).occupied) throw survivalError('BED_OCCUPIED', `Bed at ${formatPosition(bed.position)} is occupied`, 'minecraft.sleep', { position: bed.position }); this.currentSleepState = { state: 'READY', bed: bed.position, error: null }; await bot.sleep(block); if (!bot.isSleeping) throw survivalError('SLEEP_UNAVAILABLE', 'Minecraft rejected the sleep request', 'minecraft.sleep', { position: bed.position }); this.currentSleepState = { state: 'SLEEPING', bed: bed.position, error: null }; return { ...this.currentSleepState, verified: true }; }
...
function equipmentView(bot, destination) { const direct = bot.equipment?.[destination]; const slot = bot.getEquipmentDestSlot?.(destination); const item = direct ?? (Number.isInteger(slot) ? bot.inventory?.slots?.[slot] : null) ?? (destination === 'hand' ? bot.heldItem : null); return item ? { name: String(item.name), count: Number(item.count ?? 1), durabilityPercent: durabilityPercent(bot, item), enchanted: recoveryEnchanted(item), bindingCurse: hasBindingCurse(item) } : null; }
...
async function setOpenableState(adapter, input, open, kind) { const expected = input?.block; if (expected && !String(expected).includes(kind)) throw new ValidationError(`Expected block '${expected}' is not a ${kind}`); const client = adapter.client; if (!client || adapter.status !== 'READY') throw new ValidationError(`Cannot use '${kind}' interaction while bot is ${adapter.status}`); const position = validRecoveryPosition(input?.position); const block = client.blockAt(await blockVector(position)); if (!block || !block.name.includes(kind)) throw survivalError('BLOCK_NOT_FOUND', `${kind} was not found at ${formatPosition(position)}`, `minecraft.${open ? 'open' : 'close'}-${kind}`, { position, actual: block?.name ?? null }); if (kind === 'door' && /^iron_door$/.test(block.name)) throw survivalError('CAPABILITY_UNAVAILABLE', 'Iron doors require a redstone mechanism and cannot be directly activated', `minecraft.${open ? 'open' : 'close'}-door`, { position, block: block.name }); return adapter.interactBlock({ position, expectedBlock: block.name, action: open ? 'OPEN' : 'CLOSE', cooldownMs: input?.cooldownMs ?? 500 }); }

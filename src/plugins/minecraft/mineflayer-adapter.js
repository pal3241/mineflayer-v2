...
export class MineflayerAdapter extends EventEmitter {
  ...
  async safePillarStep(input, context) {
    const bot = this.#ready('navigation-pillar'); if (context?.signal?.aborted) throw context.signal.reason ?? new ValidationError('Pillar step cancelled'); const itemName = String(input?.item ?? '').trim(); const item = bot.inventory.items().find(entry => entry.name === itemName); if (!item) throw new ValidationError(`Scaffold item '${itemName}' is unavailable`); const start = bot.entity.position; const pillarStartY = Number(input?.pillarStartY ?? start.y); if (start.y - pillarStartY >= Number(input?.maxPillarHeight ?? 0)) throw new ValidationError('PILLAR_HEIGHT_EXCEEDED maximum pillar height reached'); const feet = start.floored(); const base = bot.blockAt(feet.offset(0, -1, 0)); const head = bot.blockAt(feet.offset(0, 1, 0)); const nextHead = bot.blockAt(feet.offset(0, 2, 0)); if (!base || isAir(base)) throw new ValidationError('PILLAR_STEP_FAILED no supporting block below bot'); if (!isAir(head) || !isAir(nextHead)) throw new ValidationError('PILLAR_STEP_FAILED pillar head clearance is blocked'); const target = base.position.offset(0, 1, 0); if (!isAir(bot.blockAt(target))) throw new ValidationError('PILLAR_STEP_FAILED pillar target is occupied'); const { Vec3 } = await import('vec3'); await bot.equip(item, 'hand'); bot.setControlState('jump', true); let placed = false; try { await waitForVerticalPosition(bot, start.y + 0.35, context?.signal); await bot.placeBlock(base, new Vec3(0, 1, 0)); const placedBlock = bot.blockAt(target); if (!placedBlock || placedBlock.name !== itemName) throw new ValidationError('PLACEMENT_NOT_VERIFIED navigation scaffold block was not found after placement'); placed = true; await waitForVerticalPosition(bot, start.y + 0.8, context?.signal); return { position: { x: target.x, y: target.y, z: target.z }, item: itemName, verified: true }; } catch (error) { if (placed) { error.details = { ...(error.details ?? {}), blockPlaced: true, position: { x: target.x, y: target.y, z: target.z }, item: itemName }; } throw error instanceof ValidationError ? error : new ValidationError(`PILLAR_STEP_FAILED ${error.message}`); } finally { bot.setControlState('jump', false); }
  }

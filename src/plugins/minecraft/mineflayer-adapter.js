...
export class MineflayerAdapter extends EventEmitter {
  ...
  async placeBlueprintBlock(input, { signal } = {}) {
    ...
    const { Vec3 } = await import('vec3'); await bot.equip(item, 'hand'); await bot.placeBlock(chosen.reference, new Vec3(-chosen.x, -chosen.y, -chosen.z));
    ...
    return { position, item: itemName, verified: true };
  }

...
export class MineflayerAdapter extends EventEmitter {
  ...
  async safePillarStep(input, context) {
    const bot = this.#ready('navigation-pillar'); if (context?.signal?.aborted) throw context.signal.reason ?? new ValidationError('Pillar step cancelled'); const itemName = String(input?.item ?? '').trim(); const item = bot.inventory.items().find(entry => entry.name === itemName); if (!item) throw new ValidationError(`Scaffold item '${itemName}' is unavailable`); const start = bot.entity.position; const base = bot.blockAt(start.floored().offset(0, -1, 0)); if (!base || isAir(base)) throw new ValidationError('PILLAR_STEP_FAILED no supporting block below bot'); const target = base.position.offset(0, 1, 0); if (!isAir(bot.blockAt(target))) throw new ValidationError('PILLAR_STEP_FAILED pillar target is occupied'); const { Vec3 } = await import('vec3'); await bot.equip(item, 'hand'); bot.setControlState('jump', true); try { await waitForVerticalPosition(bot, start.y + 0.35, context?.signal); await bot.placeBlock(base, new Vec3(0, 1, 0)); const placed = bot.blockAt(target); if (!placed || placed.name !== itemName) throw new ValidationError('PLACEMENT_NOT_VERIFIED navigation scaffold block was not found after placement'); await waitForVerticalPosition(bot, start.y + 0.8, context?.signal); return { position: { x: target.x, y: target.y, z: target.z }, item: itemName, verified: true }; } catch (error) { throw error instanceof ValidationError ? error : new ValidationError(`PILLAR_STEP_FAILED ${error.message}`); } finally { bot.setControlState('jump', false); }
  }

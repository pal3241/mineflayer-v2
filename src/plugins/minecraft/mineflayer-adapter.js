...
export class MineflayerAdapter extends EventEmitter {
  ...
  survivalStatus() { const bot=this.#ready('survival-status');const inventory=bot.inventory?.items?.()??[];const hostiles=Object.values(bot.entities??{}).filter(entity=>HOSTILE_MOBS.has(String(entity?.name??entity?.mobType??'').toLowerCase())&&entity?.position&&distance3(entity.position,bot.entity.position)<=12);const feet=bot.blockAt?.(bot.entity.position.floored());return {health:Number(bot.health??0),food:Number(bot.food??0),oxygen:bot.oxygenLevel===undefined?null:Number(bot.oxygenLevel),onFire:Boolean(bot.entity?.isOnFire??bot.isOnFire),inLava:feet?.name==='lava',inWater:Boolean(bot.entity?.isInWater??feet?.name==='water'),hostileCount:hostiles.length,foodItems:inventory.filter(item=>isFoodItem(bot,item)).reduce((sum,item)=>sum+Number(item.count??0),0),dimension:String(bot.game?.dimension??'overworld'),inventory:inventory.map(item=>({name:item.name,count:item.count}))}; }

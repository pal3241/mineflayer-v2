...
async function waitForBridgeStability(bot, position) { const started = Date.now(); let stableSamples = 0; while (Date.now() - started < 2_000) { const current = bot.entity?.position; if (!current) throw new ValidationError('Bridge movement stability could not be verified'); if (distance3(current, position) <= 0.75) stableSamples++; else stableSamples = 0; if (stableSamples >= 3) return; await delay(50); } throw new NavigationError('BRIDGE_UNSTABLE_POSITION', `Bridge position was not stable at ${formatPosition(position)}`, { position }); }

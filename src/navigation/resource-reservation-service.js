...
function reservationReason(value) { const reason = String(value ?? 'NAVIGATION_SCAFFOLD').trim().toUpperCase(); if (!REASONS.has(reason)) throw new NavigationError('SCAFFOLD_RESERVATION_FAILED', `Unsupported resource reservation reason '${reason}'`, { reason }); return reason; }

      ...
      try {
        ...
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'bots' && parts[3]) {
          ...
          if (req.method === 'POST' && parts[4] === 'actions' && parts[5]) {
            const input = await body(req); const capability = ({ navigate: 'minecraft.navigation', move: 'minecraft.smart-movement', collect: 'minecraft.collection', follow: 'minecraft.follow-player', come: 'minecraft.come', farm: 'minecraft.farming', deforest: 'minecraft.deforestation', reforest: 'minecraft.reforestation', combat: 'minecraft.combat', sethome: 'minecraft.set-home', home: 'minecraft.home', craft: 'minecraft.crafting', chat: 'minecraft.chat', observe: 'minecraft.observation', stop: 'minecraft.stop' })[parts[5]];
            if (!capability) throw new ValidationError(`Unsupported action '${parts[5]}'`);
            if (['collect', 'craft', 'farm', 'deforest', 'reforest', 'combat'].includes(parts[5])) { if (parts[5] === 'collect' && (typeof input.block !== 'string' || !input.block.trim())) throw new ValidationError('Collect action requires block'); if (parts[5] === 'craft' && (typeof input.item !== 'string' || !input.item.trim())) throw new ValidationError('Craft action requires item'); const bot = this.application.bots.get(parts[3]).snapshot(); const text = parts[5] === 'collect' ? `collect ${input.block} ${input.count ?? 1}` : parts[5] === 'craft' ? `craft ${input.item} ${input.count ?? 1}` : parts[5] === 'farm' ? `farm ${input.crop ?? 'wheat'} ${input.count ?? 16}` : parts[5] === 'deforest' ? `deforest ${input.log ?? 'any'} ${input.count ?? 1}` : parts[5] === 'reforest' ? `reforest ${input.count ?? 8}` : `${input.mode ?? 'guard'} ${input.radius ?? 16}`; return send(200, { data: await this.application.coordinator.coordinate({ text, selector: `bot:${bot.metadata.commandAlias ?? bot.name}`, actor: 'api' }) }); }

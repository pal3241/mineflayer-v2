  ...
  async start() {
    ...
    this.server = createServer(async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && url.pathname === '/api/v1/settings/reset') return send(200, { data: this.application.resetRuntimeSettings() });
        ...
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'bots' && parts[3]) {
          ...
          if (req.method === 'POST' && parts[4] === 'actions' && parts[5]) {
            ...
            if (['collect', 'craft', 'survey', 'farm', 'deforest', 'reforest', 'combat'].includes(parts[5])) { if (parts[5] === 'collect' && (typeof input.block !== 'string' || !input.block.trim())) throw new ValidationError('Collect action requires block'); if (parts[5] === 'craft' && (typeof input.item !== 'string' || !input.item.trim())) throw new ValidationError('Craft action requires item'); const bot = this.application.bots.get(parts[3]).snapshot(); const text = parts[5] === 'collect' ? `collect ${input.block} ${input.count ?? 1}` : parts[5] === 'craft' ? `craft ${input.item} ${input.count ?? 1}` : parts[5] === 'survey' ? `survey ${input.radius ?? 64}` : parts[5] === 'farm' ? `farm ${input.crop ?? 'wheat'} ${input.count ?? 16}` : parts[5] === 'deforest' ? `deforest ${input.log ?? 'any'} ${input.count ?? 1}` : parts[5] === 'reforest' ? `reforest ${input.count ?? 8}` : `${input.mode ?? 'guard'} ${input.radius ?? 16}`; return send(200, { data: await this.application.coordinator.coordinate({ text, selector: `bot:${bot.metadata.commandAlias ?? bot.name}`, actor: 'api' }) }); }

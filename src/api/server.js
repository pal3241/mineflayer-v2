  ...
  async start() {
    ...
    this.server = createServer(async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'GET' && url.pathname === '/api/v1/settings') return send(200, { data: { llm: { ...this.application.llm.settings(), status: this.application.llm.status() }, log: this.application.logger.status(), autonomy: this.application.autonomy.status(), memory: await this.application.memorySettings() } });

  ...
  async start() {
    ...
    this.server = createServer(async (req, res) => {
      ...
      try {
        ...
        const status = ['NOT_FOUND', 'BOT_NOT_FOUND', 'NAVIGATION_NOT_ACTIVE'].includes(error.code) ? 404 : ['CONFLICT', 'CANCELLED', 'NAVIGATION_BUSY', 'NAVIGATION_CANCELLED', 'BOT_NOT_READY'].includes(error.code) ? 409 : ['VALIDATION_ERROR', 'INVALID_TARGET', 'INVALID_GROUP', 'INVALID_FORMATION'].includes(error.code) ? 400 : error.code === 'NAVIGATION_TIMEOUT' ? 504 : ['PATH_NOT_FOUND', 'ARRIVAL_NOT_VERIFIED', 'NAVIGATION_FAILED', 'GROUP_NAVIGATION_FAILED', 'TARGET_UNAVAILABLE'].includes(error.code) ? 422 : 500;

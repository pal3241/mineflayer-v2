  ...
  async start() {
    ...
    this.server = createServer(async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'GET' && url.pathname === '/api/v1/combat/doctrines') return send(200, { data: await this.application.combat.doctrines() });
        if (req.method === 'POST' && url.pathname === '/api/v1/combat/doctrines') return send(201, { data: await this.application.combat.ingestDoctrine(await body(req)) });
        ...
        if (req.method === 'GET' && url.pathname === '/api/v1/logistics/workshops') return send(200, { data: await this.application.workshops.known({ worldKey: url.searchParams.get('worldKey'), dimension: url.searchParams.get('dimension'), kind: url.searchParams.get('kind') ?? undefined, position: url.searchParams.has('x') ? { x: Number(url.searchParams.get('x')), y: Number(url.searchParams.get('y')), z: Number(url.searchParams.get('z')) } : null }) });
        ...
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'building' && parts[3] === 'blueprints' && parts[4]) {
          ...
          if (req.method === 'POST' && parts[5] === 'placement') return send(200, { data: await this.application.building.place(blueprintId, await body(req)) });
        ...
        if (req.method === 'POST' && url.pathname === '/api/v1/navigation/group/move') return send(200, { data: await this.application.navigation.moveGroup(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/navigation/cancel') return send(200, { data: await this.application.navigation.cancel(await body(req)) });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/map') return send(200, { data: await this.application.territory.map({ worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined }) });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/at') return send(200, { data: await this.application.territory.at({ worldKey: url.searchParams.get('worldKey'), dimension: url.searchParams.get('dimension'), position: { x: Number(url.searchParams.get('x')), y: Number(url.searchParams.get('y')), z: Number(url.searchParams.get('z')) } }) });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/regions') return send(200, { data: await this.application.territory.list({ worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined, type: url.searchParams.get('type') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/regions') return send(201, { data: await this.application.territory.create(await body(req)) });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/intelligence') return send(200, { data: await this.application.territoryIntelligence.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/signals') return send(200, { data: await this.application.territoryIntelligence.signals({ worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined, kind: url.searchParams.get('kind') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/signals/resource') return send(201, { data: await this.application.territoryIntelligence.recordResource(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/signals/danger') return send(201, { data: await this.application.territoryIntelligence.recordDanger(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/frontiers/seed') return send(201, { data: await this.application.exploration.seedFrontiers(await body(req)) });
        if (req.method === 'GET' && url.pathname === '/api/v1/exploration/status') return send(200, { data: await this.application.exploration.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/exploration/missions') return send(200, { data: await this.application.exploration.list({ status: url.searchParams.get('status') ?? undefined, botId: url.searchParams.get('botId') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/exploration/missions') return send(201, { data: await this.application.exploration.createMission(await body(req)) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'exploration' && parts[3] === 'missions' && parts[4]) { if (req.method === 'GET' && parts.length === 5) return send(200, { data: await this.application.exploration.get(parts[4]) }); if (req.method === 'POST' && parts[5] === 'execute') return send(200, { data: await this.application.exploration.execute(parts[4]) }); if (req.method === 'POST' && parts[5] === 'cancel') return send(200, { data: await this.application.exploration.cancel(parts[4]) }); }
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/expansions/status') return send(200, { data: await this.application.expansion.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/expansions') return send(200, { data: await this.application.expansion.list({ status: url.searchParams.get('status') ?? undefined, source: url.searchParams.get('source') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/expansions') return send(201, { data: await this.application.expansion.propose(await body(req)) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'territory' && parts[3] === 'expansions' && parts[4] && parts[4] !== 'status') { if (req.method === 'GET' && parts.length === 5) return send(200, { data: await this.application.expansion.get(parts[4]) }); if (req.method === 'POST' && parts[5] === 'revalidate') return send(200, { data: await this.application.expansion.revalidate(parts[4], await body(req)) }); if (req.method === 'POST' && parts[5] === 'approve') return send(200, { data: await this.application.expansion.approve(parts[4], await body(req)) }); if (req.method === 'POST' && parts[5] === 'apply') return send(200, { data: await this.application.expansion.apply(parts[4]) }); if (req.method === 'POST' && parts[5] === 'cancel') return send(200, { data: await this.application.expansion.cancel(parts[4]) }); }
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/logistics/rank') return send(200, { data: await this.application.territoryLogistics.rankStorages(await body(req)) });
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/threats/status') return send(200, { data: await this.application.threats.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/ml/environment/status') return send(200, { data: await this.application.environmentModel.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/ml/environment/areas') return send(200, { data: await this.application.environmentModel.areas({ worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/environment/observe') return send(201, { data: await this.application.environmentModel.observe(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/environment/train') return send(200, { data: await this.application.environmentModel.train(await body(req)) });
        if (req.method === 'GET' && url.pathname === '/api/v1/ml/local-command/status') return send(200, { data: this.application.localBrain.status() });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/local-command/train') return send(200, { data: await this.application.localBrain.train(await body(req)) });
        if (req.method === 'PATCH' && url.pathname === '/api/v1/ml/local-command/settings') return send(200, { data: this.application.localBrain.configure(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/local-command/samples') return send(201, { data: await this.application.localBrain.learn(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/local-dialogue/samples') return send(201, { data: await this.application.localBrain.learnDialogue(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/ml/local-dialogue/respond') { const input = await body(req); return send(200, { data:await this.application.localBrain.respond(input.text, input.context) }); }
        if (req.method === 'GET' && url.pathname === '/api/v1/territory/threats') return send(200, { data: await this.application.threats.list({ status: url.searchParams.get('status') ?? undefined, level: url.searchParams.get('level') ?? undefined }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/territory/threats') return send(201, { data: await this.application.threats.detect(await body(req)) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'territory' && parts[3] === 'threats' && parts[4] && parts[4] !== 'status') { if (req.method === 'GET' && parts.length === 5) return send(200, { data: await this.application.threats.get(parts[4]) }); if (req.method === 'POST' && parts[5] === 'resolve') return send(200, { data: await this.application.threats.resolve(parts[4], await body(req)) }); }
        if (req.method === 'GET' && url.pathname === '/api/v1/resilience/status') return send(200, { data: await this.application.resilience.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/resilience/incidents') return send(200, { data: await this.application.resilience.incidents() });
        if (req.method === 'GET' && url.pathname === '/api/v1/resilience/dead-letters') return send(200, { data: await this.application.resilience.deadLetters() });
        if (req.method === 'GET' && url.pathname === '/api/v1/resilience/recovery') return send(200, { data: await this.application.resilience.recoveryQueue() });
        if (req.method === 'POST' && url.pathname === '/api/v1/resilience/emergencies') return send(201, { data: await this.application.resilience.raise(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/resilience/dependencies') return send(200, { data: await this.application.resilience.reportDependency(await body(req)) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'resilience' && parts[3] === 'emergencies' && parts[4] && req.method === 'POST' && parts[5] === 'resolve') return send(200, { data: await this.application.resilience.resolve(parts[4], await body(req)) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'territory' && parts[3] === 'regions' && parts[4]) { if (req.method === 'GET') return send(200, { data: await this.application.territory.get(parts[4]) }); if (req.method === 'PATCH') return send(200, { data: await this.application.territory.update(parts[4], await body(req)) }); if (req.method === 'DELETE') return send(200, { data: await this.application.territory.remove(parts[4]) }); }
        if (req.method === 'GET' && url.pathname === '/api/v1/dashboard/snapshot') { const [health, autonomyObjectives, profiles, logistics, memorySettings] = await Promise.all([this.application.health.check(), this.application.autonomy.objectives(), this.application.botProfiles.list(), this.application.logistics.status(), this.application.memorySettings()]); const bots = this.application.bots.list().map(bot => ({ ...bot, profile: profiles.find(profile => profile.id === bot.id) ?? null })); return send(200, { data: { health, bots, goals: this.application.goals.list(), admins: this.application.admins.list(), ai: this.application.coordinator.status(), taskQueue: this.application.executor.status(), logistics, diagnostics: processDiagnostics(), settings: { llm: { ...this.application.llm.settings(), status: this.application.llm.status() }, localBrain: this.application.localBrain.status(), environmentModel: await this.application.environmentModel.status(), log: this.application.logger.status(), autonomy: this.application.autonomy.status(), acquisition: this.application.acquisition.settings(), survival: this.application.survival.settings(), memory: memorySettings }, autonomyObjectives } }); }
        if (req.method === 'GET' && url.pathname === '/api/v1/settings') return send(200, { data: { llm: { ...this.application.llm.settings(), status: this.application.llm.status() }, log: this.application.logger.status(), autonomy: this.application.autonomy.status(), acquisition: this.application.acquisition.settings(), survival: this.application.survival.settings(), movement: this.application.navigationSettings.get({ scope: url.searchParams.get('scope') ?? 'GLOBAL', botId: url.searchParams.get('botId') }), memory: await this.application.memorySettings() } });
        if (req.method === 'GET' && url.pathname === '/api/v1/navigation/settings') return send(200, { data: this.application.navigationSettings.get({ scope: url.searchParams.get('scope') ?? 'GLOBAL', botId: url.searchParams.get('botId') }) });
        if (req.method === 'PATCH' && url.pathname === '/api/v1/navigation/settings') return send(200, { data: await this.application.navigationSettings.configure(await body(req)) });
        if (req.method === 'DELETE' && url.pathname === '/api/v1/navigation/settings') return send(200, { data: await this.application.navigationSettings.removeOverride({ scope: url.searchParams.get('scope') ?? 'BOT', botId: url.searchParams.get('botId') }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/navigation/settings/preset') return send(200, { data: await this.application.navigationSettings.applyPreset(await body(req)) });
        ...
        if (req.method === 'GET' && url.pathname === '/api/v1/memory/episodic') return send(200, { data: await this.application.episodicMemory.search({ text: url.searchParams.get('q') ?? '', episodeType: url.searchParams.get('episodeType') ?? undefined, outcome: url.searchParams.get('outcome') ?? undefined, botId: url.searchParams.get('botId') ?? undefined, worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined, limit: url.searchParams.get('limit') ?? 20 }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/memory/episodic') return send(201, { data: await this.application.episodicMemory.remember(await body(req)) });
        if (req.method === 'GET' && parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'memory' && parts[3] === 'episodic' && parts[4] && parts.length === 5) return send(200, { data: await this.application.episodicMemory.find(decodeURIComponent(parts[4])) });
        if (req.method === 'GET' && url.pathname === '/api/v1/memory/knowledge') return send(200, { data: await this.application.knowledge.search({ text: url.searchParams.get('q') ?? '', knowledgeType: url.searchParams.get('knowledgeType') ?? undefined, status: url.searchParams.get('status') ?? undefined, intent: url.searchParams.get('intent') ?? undefined, worldKey: url.searchParams.get('worldKey') ?? undefined, dimension: url.searchParams.get('dimension') ?? undefined, limit: url.searchParams.get('limit') ?? 10 }) });
        if (req.method === 'POST' && url.pathname === '/api/v1/memory/knowledge') return send(201, { data: await this.application.knowledge.remember(await body(req)) });
        if (req.method === 'POST' && url.pathname === '/api/v1/memory/knowledge/synthesize') return send(200, { data: await this.application.knowledge.synthesize((await body(req)).action) });
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'memory' && parts[3] === 'knowledge' && parts[4] && parts.length >= 5) {
          const knowledgeId = decodeURIComponent(parts[4]);
          if (req.method === 'GET' && parts.length === 5) return send(200, { data: await this.application.knowledge.find(knowledgeId) });
          if (req.method === 'PATCH' && parts.length === 5) return send(200, { data: await this.application.knowledge.revise(knowledgeId, await body(req)) });
          if (req.method === 'POST' && parts[5] === 'feedback') return send(200, { data: await this.application.knowledge.feedback(knowledgeId, await body(req)) });
        }
        if (req.method === 'GET' && url.pathname === '/api/v1/memory/events/status') return send(200, { data: await this.application.memoryEventStream.status() });
        if (req.method === 'GET' && url.pathname === '/api/v1/memory/events') return send(200, { data: await this.application.memoryEventStream.query({ afterSequence: url.searchParams.get('afterSequence') ?? 0, beforeSequence: url.searchParams.get('beforeSequence') ?? undefined, types: url.searchParams.get('types') ?? undefined, layers: url.searchParams.get('layers') ?? undefined, correlationId: url.searchParams.get('correlationId') ?? undefined, limit: url.searchParams.get('limit') ?? 100 }) });
        ...
        if (req.method === 'DELETE' && parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'logistics' && parts[3] === 'storages' && parts[4] && parts.length === 5) return send(200, { data: await this.application.logistics.removeStorage({ storageId: decodeURIComponent(parts[4]) }) });
        ...
        if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'help' && parts[3] === 'sessions' && parts[4]) {
          ...
          if (req.method === 'GET' && parts[5] === 'workers') return send(200, { data: await this.application.help.workerStates(parts[4]) });
          if (req.method === 'POST' && parts[5] === 'rebalance') return send(200, { data: await this.application.help.rebalanceSession({ sessionId: parts[4], ...(await body(req)) }) });
          if (req.method === 'POST' && parts[5] === 'steal') return send(200, { data: await this.application.help.stealWork({ sessionId: parts[4], ...(await body(req)) }) });
          if (req.method === 'POST' && parts[5] === 'pause') return send(200, { data: await this.application.help.pause({ sessionId: parts[4], ...(await body(req)) }) });
          if (req.method === 'POST' && parts[5] === 'resume') return send(200, { data: await this.application.help.resume({ sessionId: parts[4], ...(await body(req)) }) });
        }
        ...
        const status = ['NOT_FOUND', 'BOT_NOT_FOUND', 'NAVIGATION_NOT_ACTIVE'].includes(error.code) ? 404 : ['CONFLICT', 'CANCELLED', 'NAVIGATION_BUSY', 'NAVIGATION_CANCELLED', 'BOT_NOT_READY'].includes(error.code) ? 409 : ['VALIDATION_ERROR', 'INVALID_TARGET', 'INVALID_GROUP', 'INVALID_FORMATION', 'INVALID_CORRIDOR'].includes(error.code) ? 400 : error.code === 'NAVIGATION_TIMEOUT' ? 504 : ['PATH_NOT_FOUND', 'ARRIVAL_NOT_VERIFIED', 'NAVIGATION_FAILED', 'GROUP_NAVIGATION_FAILED', 'TARGET_UNAVAILABLE'].includes(error.code) ? 422 : 500;

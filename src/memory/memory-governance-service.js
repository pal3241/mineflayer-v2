...
function migrate(original, provider, ttlMs) {
  ...
  const embeddingValid = typeof provider.isCompatible === 'function' && provider.isCompatible(original.embedding);
  const representation = embeddingValid ? representationFrom(original.embedding) : provider.embed(content);
...
function representationFrom(embedding) { const { model, version, dimensions, generatedAt, ...representation } = embedding; return structuredClone(representation); }

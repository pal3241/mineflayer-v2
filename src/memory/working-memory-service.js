...
export function createWorkingMemoryService({ repository, events, governance = null, maxRecords = 1000, ttlMs = 1_800_000 }) {
  ...
  const purgeExpired = async (reason = 'ttl-expired') => { const now = Date.now(); const expired = (await repository.list()).filter(item => Date.parse(item.expiresAt) <= now); for (const item of expired) { await repository.delete(item.id); await audit('PURGED', item.id, reason, { taskId: item.taskId, botId: item.botId }); await events?.publish('memory.working.purged', { id: item.id, taskId: item.taskId, botId: item.botId, reason }, { source: 'working-memory', correlationId: item.taskId }); } return expired.length; };
  const initialize = async () => { const records = await repository.list(); for (const item of records) { await repository.delete(item.id); await audit('PURGED', item.id, 'restart-stale', { taskId: item.taskId, botId: item.botId }); await events?.publish('memory.working.purged', { id: item.id, taskId: item.taskId, botId: item.botId, reason: 'restart-stale' }, { source: 'working-memory', correlationId: item.taskId }); } initialized = true; return { purged: records.length, reason: 'restart-stale' }; };
  ...
  const activate = async task => {
    ...
    const saved = existing ? await repository.update(existing.id, record) : await repository.create(record); await audit(existing ? 'UPDATED' : 'ACTIVATED', saved.id, null, { taskId, botId }); await events?.publish(existing ? 'memory.working.updated' : 'memory.working.activated', saved, { source: 'working-memory', correlationId: taskId }); await enforceBound(); return structuredClone(saved);
  };
  const update = async (taskId, patch) => {
    ...
    const saved = await repository.update(current.id, record); await audit('UPDATED', saved.id, null, { taskId: saved.taskId, botId: saved.botId }); await events?.publish('memory.working.updated', saved, { source: 'working-memory', correlationId: taskId }); return structuredClone(saved);
  };
  const release = async (taskId, reason = 'task-terminal') => { const current = (await repository.list()).find(item => item.taskId === taskId); if (!current) return false; await repository.delete(current.id); await audit('RELEASED', current.id, reason, { taskId: current.taskId, botId: current.botId }); await events?.publish('memory.working.released', { id: current.id, taskId: current.taskId, botId: current.botId, reason }, { source: 'working-memory', correlationId: taskId }); return true; };
  ...
  const enforceBound = async () => { const records = (await repository.list()).sort((a, b) => a.updatedAt.localeCompare(b.updatedAt)); for (const item of records.slice(0, Math.max(0, records.length - maxRecords))) { await repository.delete(item.id); await audit('PURGED', item.id, 'capacity-limit', { taskId: item.taskId, botId: item.botId }); await events?.publish('memory.working.purged', { id: item.id, taskId: item.taskId, botId: item.botId, reason: 'capacity-limit' }, { source: 'working-memory', correlationId: item.taskId }); } };

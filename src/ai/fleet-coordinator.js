...
export class FleetCoordinator {
  ...
  async coordinate({ text, selector, actor = 'api' }) {
    ...
    this.hive.syncMembers(this.bots.list()); const memoryTargets = this.#select(selector ?? 'auto'); const targetRuntime = memoryTargets.length ? this.bots.get(memoryTargets[0].id) : null; const rawMemories = targetRuntime ? await this.memory?.forBot(targetRuntime, { limit: 12 }) : []; const working = targetRuntime && this.workingMemory ? await this.workingMemory.list({ botId: targetRuntime.id, limit: 8 }) : []; const identity = targetRuntime ? serverIdentity(targetRuntime.options) : null; const episodic = targetRuntime && this.episodicMemory ? await this.episodicMemory.search({ text, botId: targetRuntime.id, worldKey: `${identity.host}:${identity.port}`, dimension: targetRuntime.adapter.snapshot().dimension, limit: 6 }) : []; const semantic = targetRuntime ? await this.semanticMemory.recall({ text, worldKey: `${identity.host}:${identity.port}`, dimension: targetRuntime.adapter.snapshot().dimension, sourceBotId: targetRuntime.id, limit: 8 }) : []; const memories = [...working.map(memory => ({ kind: 'working', taskId: memory.taskId, goalId: memory.goalId, currentAction: memory.currentAction, target: memory.target, progress: memory.progress, cargo: memory.cargo, route: memory.route, updatedAt: memory.updatedAt })), ...episodic.map(memory => ({ kind: 'episodic', episodeType: memory.episodeType, action: memory.action, outcome: memory.outcome, position: memory.position, cause: memory.cause, lesson: memory.lesson, relevance: memory.relevance, occurredAt: memory.occurredAt })), ...rawMemories.map(memory => ({ kind: 'world', type: memory.type, name: memory.name, position: memory.position, confidence: memory.confidence, updatedAt: memory.updatedAt })), ...semantic.map(memory => ({ kind: 'semantic', type: memory.type, content: memory.content, confidence: memory.confidence, relevance: memory.relevance, source: memory.source }))];

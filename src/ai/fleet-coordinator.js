...
export class FleetCoordinator {
  ...
  async coordinate({ text, selector, actor = 'api' }) {
    ...
    this.hive.syncMembers(this.bots.list()); const memoryTargets = this.#select(selector ?? 'auto'); const targetRuntime = memoryTargets.length ? this.bots.get(memoryTargets[0].id) : null; const rawMemories = targetRuntime ? await this.memory?.forBot(targetRuntime, { limit: 12 }) : []; const working = targetRuntime && this.workingMemory ? await this.workingMemory.list({ botId: targetRuntime.id, limit: 8 }) : []; const identity = targetRuntime ? serverIdentity(targetRuntime.options) : null; const scope = targetRuntime ? { worldKey: `${identity.host}:${identity.port}`, dimension: targetRuntime.adapter.snapshot().dimension } : {}; const verified = targetRuntime && this.memoryIntegrity ? await this.memoryIntegrity.retrieve({ actorBotId: targetRuntime.id, teamId: targetRuntime.metadata?.teamId ?? null, ...scope }) : []; const episodic = targetRuntime && this.episodicMemory ? await this.episodicMemory.search({ text, botId: targetRuntime.id, ...scope, limit: 6 }) : []; const learned = targetRuntime && this.knowledge ? await this.knowledge.search({ text, status: 'ACTIVE', ...scope, limit: 6 }) : []; const semantic = targetRuntime ? await this.semanticMemory.recall({ text, ...scope, sourceBotId: targetRuntime.id, limit: 8 }) : []; const memories = boundMemoryContext([...working.map(memory => ({ kind: 'working', taskId: memory.taskId, goalId: memory.goalId, currentAction: memory.currentAction, target: memory.target, progress: memory.progress, cargo: memory.cargo, route: memory.route, updatedAt: memory.updatedAt })), ...verified.map(memory => ({ kind: 'verified', claimKey: memory.claimKey, layer: memory.layer, value: memory.value, confidence: memory.confidence, authorityScore: memory.authorityScore, observedAt: memory.observedAt })), ...learned.map(memory => ({ kind: 'knowledge', knowledgeType: memory.knowledgeType, intent: memory.intent, name: memory.name, preconditions: memory.preconditions, steps: memory.steps, recommendations: memory.recommendations, avoidConditions: memory.avoidConditions, confidence: memory.confidence, relevance: memory.relevance, version: memory.version })), ...episodic.map(memory => ({ kind: 'episodic', episodeType: memory.episodeType, action: memory.action, outcome: memory.outcome, position: memory.position, cause: memory.cause, lesson: memory.lesson, relevance: memory.relevance, occurredAt: memory.occurredAt })), ...rawMemories.map(memory => ({ kind: 'world', type: memory.type, name: memory.name, position: memory.position, confidence: memory.confidence, updatedAt: memory.updatedAt })), ...semantic.map(memory => ({ kind: 'semantic', type: memory.type, content: memory.content, confidence: memory.confidence, relevance: memory.relevance, source: memory.source }))]);
    const intent = await this.gateway.interpret(text, { selector, fleet: this.fleetView(), memories, conversationId: `${actor}:${selector ?? 'auto'}` });
    const ranked = await rankByPrediction(this.#select(intent.selector), intent.intent, this.ml); if (!ranked.length) throw new ConflictError(`No available bots match '${intent.selector}'`); const best=ranked[0].prediction; if(best.sampleCount>=5&&best.confidence>=0.5&&best.failureProbability>=0.75)throw new ConflictError(`Task '${intent.intent}' is predicted to fail for every eligible bot (${Math.round(best.failureProbability*100)}% risk)`); const selected=ranked.map(item=>item.bot);
    const singleton = ['register_storage', 'retrieve', 'stock'].includes(intent.intent); const distributed = ['collect', 'craft', 'smelt', 'store', 'farm', 'deforest', 'reforest'].includes(intent.intent) && intent.selector !== 'auto'; const targets = singleton || intent.selector === 'auto' ? selected.slice(0, 1) : distributed ? selected.slice(0, Math.min(selected.length, intent.count)) : selected; const counts = distribute(intent.count, targets.length);
    this.#reserve(targets.map(bot => bot.id));
    const started = performance.now(); try {
      await this.events.publish('coordinator.requested', { actor, text, intent, targets: targets.map(bot => bot.id), taskSuccessPredictions:ranked.filter(item=>targets.some(bot=>bot.id===item.bot.id)).map(item=>({botId:item.bot.id,...item.prediction})) }, { source: 'fleet-coordinator' });
  ...
  async #execute(botId, intent) {
    ...
    if (intent.intent === 'move') { const server = serverIdentity(runtime.options); const area = await this.environmentModel?.areaAt?.({ worldKey:`${server.host}:${server.port}`, dimension:adapter.snapshot().dimension, position:{x:intent.x,y:intent.y,z:intent.z} }); if (area?.classification === 'DANGEROUS' && area.confidence >= 0.5) throw new ConflictError(`Target area is learned as dangerous (${Math.round(area.dangerScore * 100)}% risk, ${area.observationCount} observations)`); return adapter.smartMove({ x: intent.x, y: intent.y, z: intent.z, movement }); }
...
async function rankByPrediction(bots, intent, ml) { const candidates = await Promise.all(bots.map(async (bot, index) => ({ bot, index, prediction: await ml.predict({ botId: bot.id, intent, features: botFeatures(bot, intent, bots.length) }) }))); return candidates.sort((left, right) => right.prediction.prediction - left.prediction.prediction || left.index - right.index); }
function botFeatures(bot, intent, fleetSize) { const runtime=bot.runtime??bot;const inventory = runtime.inventorySummary ?? []; return { intent, className: String(bot.metadata?.className ?? 'worker'), health:Number(runtime.health??0), food:Number(runtime.food??0), distance:Number(runtime.distance??0), hostileCount:Number(runtime.hostileCount??0), night:Boolean(runtime.night), dimension:String(runtime.dimension??'overworld'), inventoryStacks: inventory.length, hasTool: inventory.some(item => /_(pickaxe|axe|hoe|sword)$/.test(item.name)), fleetSize }; }
function band(value, size) { const number = Number(value); return Number.isFinite(number) ? Math.floor(number / size) * size : -1; }
function boundMemoryContext(items, maxItems = 5, maxCharacters = 2500) { const result = []; let characters = 0; for (const item of items) { if (result.length >= maxItems) break; const size = JSON.stringify(item).length; if (characters + size > maxCharacters) continue; result.push(item); characters += size; } return result; }

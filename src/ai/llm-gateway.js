...
export class LlmGateway {
  ...
  settings() { const openRouterKeys = uniqueKeys(this.config.openRouterApiKeys); const nvidiaKeys = uniqueKeys(this.config.nvidiaApiKeys); return { provider: this.config.provider, openrouter: { endpoint: this.config.openRouterEndpoint ?? 'https://openrouter.ai/api/v1', model: this.config.openRouterModel ?? 'openrouter/auto', configuredKeys: [0, 1, 2].map(index => Boolean(openRouterKeys[index])) }, nvidia: { endpoint: this.config.nvidiaEndpoint ?? 'https://integrate.api.nvidia.com/v1', model: this.config.nvidiaModel ?? 'nvidia/nemotron-3-nano-30b-a3b', configuredKeys: [0, 1, 2].map(index => Boolean(nvidiaKeys[index])) }, timeoutMs: this.config.timeoutMs ?? 90_000, maxTokens: 256 }; }
  configure(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) throw new ValidationError('LLM settings must be an object'); const provider = String(input.provider); if (!['none', 'openrouter', 'nvidia'].includes(provider)) throw new ValidationError('LLM provider must be none, openrouter, or nvidia');
    const openRouterModel = validateSetting(input.openRouterModel, 'OpenRouter model', 160); const openRouterEndpoint = validateEndpoint(input.openRouterEndpoint, 'OpenRouter endpoint'); const nvidiaModel = validateSetting(input.nvidiaModel, 'NVIDIA NIM model', 160); const nvidiaEndpoint = validateEndpoint(input.nvidiaEndpoint, 'NVIDIA NIM endpoint');
    ...
    const timeoutMs = Number(input.timeoutMs ?? this.config.timeoutMs ?? 90_000); if (!Number.isInteger(timeoutMs) || timeoutMs < 5_000 || timeoutMs > 300_000) throw new ValidationError('LLM timeoutMs must be an integer from 5000 to 300000'); const config = { ...this.config, provider, openRouterEndpoint, openRouterModel, openRouterApiKeys, nvidiaEndpoint, nvidiaModel, nvidiaApiKeys, timeoutMs }; const resolved = buildProvider(config); if (!resolved && provider !== 'none') throw new ValidationError(`LLM provider '${provider}' is missing a required endpoint, model, or API key`);

...
const INTENTS = new Set(['collect', 'craft', 'smelt', 'follow', 'come', 'move', 'set_home', 'home', 'farm', 'deforest', 'reforest', 'combat', 'survey', 'register_storage', 'store', 'retrieve', 'stock', 'remember', 'place', 'status', 'converse']);

export class OpenAICompatibleProvider {
  constructor({ endpoint, apiKey, apiKeys = [], model, structuredOutput = true, headers = {}, timeoutMs = 90_000, rotateOn = [401, 402, 429] }) { this.endpoint = endpoint.replace(/\/$/, ''); this.keys = [...new Set([...apiKeys, apiKey].filter(Boolean))].map(key => ({ key, cooldownUntil: 0 })); this.model = model; this.structuredOutput = structuredOutput; this.headers = headers; this.timeoutMs = timeoutMs; this.rotateOn = new Set(rotateOn); this.keyIndex = 0; }
  async complete(messages, schema) {
    const request = (structured, apiKey) => fetch(`${this.endpoint}/chat/completions`, { method: 'POST', signal: AbortSignal.timeout(this.timeoutMs), headers: { 'content-type': 'application/json', ...(apiKey ? { authorization: `Bearer ${apiKey}` } : {}), ...this.headers },
      body: JSON.stringify({ model: this.model, messages, temperature: 0, max_tokens: 256, ...(structured ? { response_format: { type: 'json_schema', json_schema: { name: 'minehive_command', strict: true, schema } } } : {}) }) });
...
export class OllamaProvider {
  constructor({ endpoint = 'http://127.0.0.1:11434', model, timeoutMs = 90_000 }) { this.endpoint = endpoint.replace(/\/$/, ''); this.model = model; this.timeoutMs = timeoutMs; }
  async complete(messages, schema) {
    const response = await fetch(`${this.endpoint}/api/chat`, { method: 'POST', signal: AbortSignal.timeout(this.timeoutMs), headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: this.model, messages, stream: false, format: schema, options: { temperature: 0 } }) });
    if (!response.ok) throw new Error(`Ollama HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const payload = await response.json(); return payload.message?.content ?? '';
  }
  status() { return { local: true, keyCount: 0, activeKey: null, availableKeys: 0 }; }
}
...
export class LlmGateway {
  ...
  configure(input) {
    ...
    const openRouterModel = validateSetting(input.openRouterModel, 'OpenRouter model', 160); const openRouterEndpoint = validateEndpoint(input.openRouterEndpoint, 'OpenRouter endpoint'); const nvidiaModel = validateSetting(input.nvidiaModel, 'NVIDIA NIM model', 160); const nvidiaEndpoint = validateEndpoint(input.nvidiaEndpoint, 'NVIDIA NIM endpoint'); const ollamaModel = validateSetting(input.ollamaModel, 'Ollama model', 160); const ollamaEndpoint = validateEndpoint(input.ollamaEndpoint, 'Ollama endpoint');
    const openRouterApiKeys = configuredKeys(this.config.openRouterApiKeys, input.openRouterApiKeys, input.clearOpenRouterKeys, 'OpenRouter'); const nvidiaApiKeys = configuredKeys(this.config.nvidiaApiKeys, input.nvidiaApiKeys, input.clearNvidiaKeys, 'NVIDIA NIM');
    const timeoutMs = Number(input.timeoutMs ?? this.config.timeoutMs ?? 90_000); if (!Number.isInteger(timeoutMs) || timeoutMs < 5_000 || timeoutMs > 300_000) throw new ValidationError('LLM timeoutMs must be an integer from 5000 to 300000'); const config = { ...this.config, provider, openRouterEndpoint, openRouterModel, openRouterApiKeys, nvidiaEndpoint, nvidiaModel, nvidiaApiKeys, ollamaEndpoint, ollamaModel, timeoutMs }; const resolved = buildProvider(config); if (!resolved && provider !== 'none') throw new ValidationError(`LLM provider '${provider}' is missing a required endpoint, model, or API key`);
  ...
  async interpret(text, context = {}) {
    ...
    if (localStatus?.enabled && localStatus.mode === 'manual') return this.#local(text, context, deterministic);
    if (deterministic.intent !== 'converse') return deterministic;
    if (!this.provider) return localStatus?.enabled && localStatus.mode === 'fallback' ? this.#local(text, context, deterministic) : deterministic;
    ...
    catch (error) { this.logger?.warn('llm.interpretation.fallback', { error: error.message }); if (localStatus?.enabled && localStatus.mode === 'fallback') return this.#local(text, context, deterministic); return deterministic; }
  }
  async #local(text, context, deterministic) { const localStatus=this.localBrain.status(); if(localStatus.intentModel==='not-trained'&&deterministic.intent!=='converse')return deterministic; const prediction = this.localBrain.predict(text); const selector = context.selector; if (prediction.label === 'converse' || (prediction.confidence < 0.45 && deterministic.intent === 'converse')) { const dialogue = await this.localBrain.respond(text, context); return validateIntent({ intent:'converse', selector, reply:dialogue.reply }, selector); } if (prediction.confidence < 0.45) return deterministic; try { return neuralIntent(text, prediction.label, selector); } catch { return deterministic.intent === 'converse' ? validateIntent({ intent:'converse', selector, reply:(await this.localBrain.respond(text, context)).reply }, selector) : deterministic; } }
}
...
function buildProvider(config) {
  const openRouterKeys = uniqueKeys(config.openRouterApiKeys); if (config.provider === 'openrouter' && openRouterKeys.length) return { name: 'openrouter', provider: new OpenAICompatibleProvider({ endpoint: config.openRouterEndpoint || 'https://openrouter.ai/api/v1', apiKeys: openRouterKeys, model: config.openRouterModel || 'openrouter/auto', structuredOutput: true, headers: { 'HTTP-Referer': config.siteUrl ?? 'http://localhost', 'X-Title': 'MineHive' }, timeoutMs: config.timeoutMs }) };
  if (config.provider === 'ollama' && config.ollamaModel) return { name: 'ollama', provider: new OllamaProvider({ endpoint: config.ollamaEndpoint || 'http://127.0.0.1:11434', model: config.ollamaModel, timeoutMs: config.timeoutMs }) };
  const nvidiaKeys = uniqueKeys(config.nvidiaApiKeys); const nvidiaEndpoint = config.nvidiaEndpoint || 'https://integrate.api.nvidia.com/v1'; const hostedNvidia = new URL(nvidiaEndpoint).hostname.toLowerCase() === 'integrate.api.nvidia.com'; if (config.provider === 'nvidia' && config.nvidiaModel && (nvidiaKeys.length || !hostedNvidia)) return { name: 'nvidia', provider: new OpenAICompatibleProvider({ endpoint: nvidiaEndpoint, apiKeys: nvidiaKeys, model: config.nvidiaModel, structuredOutput: false, timeoutMs: config.timeoutMs }) };
  return null;
}

function configuredKeys(current, replacement, clear, provider) { if (clear === true) return []; if (replacement === undefined) return uniqueKeys(current); if (!Array.isArray(replacement) || replacement.length < 1 || replacement.length > 3 || replacement.some(key => typeof key !== 'string' || !key.trim() || key.length > 500)) throw new ValidationError(`${provider} keys must contain one to three non-empty keys`); return uniqueKeys(replacement.map(key => key.trim())); }
function uniqueKeys(keys) { const result = [...new Set((keys ?? []).filter(Boolean))]; if (result.length > 3) throw new ValidationError('LLM key pool must not contain more than three keys'); return result; }
...
function validateIntent(value, forcedSelector) {
  ...
  const result = { intent: value.intent, selector: forcedSelector ?? value.selector ?? 'auto', block: value.block?.toLowerCase() ?? undefined, item: value.intent === 'smelt' ? normalizeSmeltingTarget(value.item) : value.item?.toLowerCase() ?? undefined, count: normalizeCount(value.count), player: value.player ?? undefined,
  ...
  if (result.intent === 'smelt' && !result.item) throw new ValidationError('Smelt intent requires output item');
...
function deterministicIntent(text, selector = 'auto') {
  ...
  if (words.includes('smelt') || words.includes('cook') || words.includes('masak') || words.includes('lebur')) return validateIntent({ intent: 'smelt', selector, item: smeltingTarget(words), count }, selector);
...
function normalizeSmeltingTarget(value) { const item = String(value ?? '').toLowerCase().replace(/\s+/g, '_'); return (({ iron: 'iron_ingot', besi: 'iron_ingot', raw_iron: 'iron_ingot', iron_ore: 'iron_ingot', beef: 'cooked_beef', raw_beef: 'cooked_beef', sapi: 'cooked_beef', daging_sapi: 'cooked_beef', porkchop: 'cooked_porkchop', raw_porkchop: 'cooked_porkchop', babi: 'cooked_porkchop', daging_babi: 'cooked_porkchop', mutton: 'cooked_mutton', domba: 'cooked_mutton', daging_domba: 'cooked_mutton', chicken: 'cooked_chicken', ayam: 'cooked_chicken', daging_ayam: 'cooked_chicken', rabbit: 'cooked_rabbit', kelinci: 'cooked_rabbit', cod: 'cooked_cod', salmon: 'cooked_salmon', potato: 'baked_potato', kentang: 'baked_potato', kelp: 'dried_kelp' })[item] ?? item) || undefined; }
function smeltingTarget(words) { return normalizeSmeltingTarget(identifierAfter(words, ['smelt', 'cook', 'masak', 'lebur'])); }

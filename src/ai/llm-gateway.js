...
export class LlmGateway {
  ...
  #local(text, context, deterministic) { const prediction = this.localBrain.predict(text); const selector = context.selector; if (prediction.label === 'converse' || (prediction.confidence < 0.45 && deterministic.intent === 'converse')) { const dialogue = this.localBrain.respond(text, context); return validateIntent({ intent:'converse', selector, reply:dialogue.reply }, selector); } if (prediction.confidence < 0.45) return deterministic; try { return neuralIntent(text, prediction.label, selector); } catch { return deterministic.intent === 'converse' ? validateIntent({ intent:'converse', selector, reply:this.localBrain.respond(text, context).reply }, selector) : deterministic; } }
}

...
export class OpenAICompatibleProvider {
  ...
  async complete(messages, schema) {
    const request = (structured, apiKey) => fetch(`${this.endpoint}/chat/completions`, { method: 'POST', signal: AbortSignal.timeout(this.timeoutMs), headers: { 'content-type': 'application/json', ...(apiKey ? { authorization: `Bearer ${apiKey}` } : {}), ...this.headers },
      body: JSON.stringify({ model: this.model, messages, temperature: 0, max_tokens: 5, ...(structured ? { response_format: { type: 'json_schema', json_schema: { name: 'minehive_command', strict: true, schema } } } : {}) }) });
...
export class LlmGateway {
  ...
  configure(input) {
    ...
    const config = { ...this.config, provider, openRouterEndpoint, openRouterModel, openRouterApiKeys, nvidiaEndpoint, nvidiaModel, nvidiaApiKeys }; const resolved = buildProvider(config); if (!resolved && provider !== 'none') throw new ValidationError(`LLM provider '${provider}' is missing a required endpoint, model, or API key`);

...
export class LlmGateway {
  ...
  settings() { const keys = [...new Set([...(this.config.apiKeys ?? []), this.config.apiKey].filter(Boolean))]; return { provider: this.config.provider, endpoint: this.config.endpoint ?? '', model: this.config.model ?? '', localEndpoint: this.config.localEndpoint ?? '', localModel: this.config.localModel ?? '', configuredKeys: [0, 1, 2].map(index => Boolean(keys[index])), maxTokens: 5 }; }
  configure(input) {

...
export function createSurvivalService({ acquisition, events, logger, config, bots = null, taskControl = null }) {
  ...
  const torchLastAttempt = new Map();
  const shelterLastAttempt = new Map();
  ...
  const attach = runtime => {
    ...
    const supervise = async () => {
      ...
      try {
        ...
        if (emergency) { const reason=state.onFire||state.inLava?'FIRE_OR_LAVA':'DROWNING'; const result=await interrupt(runtime,reason,async()=>runtime.adapter.emergencySurvival?.({reason,state}),taskControl); await publish('survival.supervisor.interrupted',{priority:1,reason,result},runtime); return; }
        if (Number(state.health) <= policy.criticalHealth) { const reason=Number(state.health)<=policy.panicHealth?'PANIC_HEALTH':'LOW_HEALTH'; const result=await interrupt(runtime,reason,async()=>{const emergencyResult=await runtime.adapter.emergencySurvival?.({reason,state});const retreat=await runtime.adapter.retreatToSafety?.({reason,allies:allyPositions(runtime.bot.id,bots),pillarHeight:policy.emergencyPillarHeight});return {emergencyResult,retreat,verified:Boolean(emergencyResult?.verified||retreat?.verified)};},taskControl); await publish('survival.supervisor.interrupted',{priority:2,reason,result,requestBackup:true},runtime); return; }
        const foodItems=Number(state.foodItems??0); if(Number(state.food)<policy.eatAtFood){ if(foodItems>0){const result=await interrupt(runtime,'HUNGER',()=>runtime.adapter.eatNow?.(),taskControl);await publish('survival.supervisor.interrupted',{priority:3,reason:'HUNGER',result},runtime);return;} const last=foodLastAttempt.get(runtime.bot.id)??0;if(Date.now()-last>=60_000){foodLastAttempt.set(runtime.bot.id,Date.now());void acquisition.acquire({requesterBotId:runtime.bot.id,type:'ITEM',item:'bread',count:policy.minimumFoodItems,purpose:'survival food reserve',priority:100,consume:false}).then(result=>publish('survival.food.restocked',{result},runtime)).catch(error=>publish('survival.food.restock.failed',{error:error.message},runtime));} }
        ...
        await evaluateSleep();
        const torchLast=torchLastAttempt.get(runtime.bot.id)??0;
        if(policy.autoTorch&&Number(state.torchItems??0)>0&&Number(state.lightLevel??15)<=policy.torchLightThreshold&&Date.now()-torchLast>=policy.torchIntervalMs){torchLastAttempt.set(runtime.bot.id,Date.now());const result=await runtime.adapter.placeSafetyTorch?.({lightThreshold:policy.torchLightThreshold});await publish('survival.torch.placed',{result},runtime);}
        const shelterLast=shelterLastAttempt.get(runtime.bot.id)??0;
        if(policy.emergencyShelter&&state.isNight&&Number(state.hostileCount??0)>0&&Date.now()-shelterLast>=60_000){shelterLastAttempt.set(runtime.bot.id,Date.now());const result=await interrupt(runtime,'NIGHT_THREAT',()=>runtime.adapter.buildEmergencyShelter?.({pillarHeight:policy.emergencyPillarHeight}),taskControl);await publish('survival.shelter.built',{result},runtime);}
    ...
    const detach = () => { clearInterval(sleepTimer);clearInterval(supervisorTimer); runtime.adapter.removeListener('spawn', onSpawn); runtime.adapter.removeListener('inventoryUpdate', onInventoryUpdate); armorRunning.delete(runtime.bot.id); sleepRunning.delete(runtime.bot.id);supervisorRunning.delete(runtime.bot.id);foodLastAttempt.delete(runtime.bot.id);torchLastAttempt.delete(runtime.bot.id);shelterLastAttempt.delete(runtime.bot.id); sleepLastAttempt.delete(runtime.bot.id); releaseBed(runtime.bot.id); attached.delete(runtime.bot.id); };
...
export function normalizeSurvivalPolicy(input) {
  ...
  for (const field of ['enabled','supervisorEnabled','autoSleep','autoTorch','emergencyShelter', 'autoEquipArmor', 'preferProtection', 'preferDurability', 'allowBindingCurse', 'allowAnimalKill']) if (typeof policy[field] !== 'boolean') throw new ValidationError(`Survival policy '${field}' must be a boolean`);
  for (const [field, minimum, maximum] of [['supervisorIntervalMs',250,10_000],['criticalHealth',1,20],['panicHealth',1,20],['eatAtFood',1,20],['minimumFoodItems',1,64],['torchLightThreshold',0,15],['torchIntervalMs',1000,300_000],['emergencyPillarHeight',2,8],['minimumDurabilityPercent', 0, 100], ['minimumSheepReserve', 0, 100], ['minimumCowReserve', 0, 100], ['interactionCooldownMs', 100, 10_000], ['entitySearchDistance', 4, 128]]) if (!Number.isInteger(policy[field]) || policy[field] < minimum || policy[field] > maximum) throw new ValidationError(`Survival policy '${field}' must be an integer between ${minimum} and ${maximum}`);
...
async function interrupt(runtime,reason,operation,taskControl){const taskCheckpoint=taskControl?.beginSurvivalInterrupt?.(runtime.bot.id,reason);let checkpoint;if(typeof runtime.adapter.beginSurvivalInterrupt==='function')checkpoint=await runtime.adapter.beginSurvivalInterrupt({reason});else await runtime.adapter.stopActions?.();try{return await operation();}finally{let state=null;try{state=await runtime.adapter.survivalStatus?.();}catch{}const safe=!state||(!state.onFire&&!state.inLava&&(state.oxygen===null||state.oxygen===undefined||state.oxygen>4)&&Number(state.health??20)>0);if(safe){const resumed=await runtime.adapter.resumeSurvivalInterrupt?.({reason,checkpoint});await taskControl?.endSurvivalInterrupt?.(runtime.bot.id,{reason,adapter:resumed,checkpoint:taskCheckpoint});}}}
function allyPositions(botId,bots){if(!bots?.list)return [];return bots.list().filter(bot=>bot.id!==botId&&bot.status==='READY'&&bot.runtime?.position).map(bot=>({botId:bot.id,...bot.runtime.position,dimension:bot.runtime.dimension}));}

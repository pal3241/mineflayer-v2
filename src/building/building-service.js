...
class BuildingService {
  ...
  protocol() { return { protocol: 'minehive.building', version: '1.2', apiVersion: 'v1', transports: ['http-poll'], synchronization: { snapshot: true, delta: true, revisionField: 'revision' }, commands: ['place', 'transform', 'approve', 'build', 'pause', 'resume', 'cancel'], formats: [{ id: 'minehive-json', mode: 'native' }, { id: 'schem', mode: 'decoder-adapter' }, { id: 'litematic', mode: 'decoder-adapter' }], rendering: { inWorldGhost: 'planned', litematicaAdapter: 'placement-origin-rotation-mirror' } }; }
  async import(input) { const base = importBlueprint(input, this.config); const now = iso(); const project = { ...base, sourceBlocks: structuredClone(base.blocks), sourceOrigin: structuredClone(base.origin), transform: { rotation: 0, mirrorX: false, mirrorZ: false }, revision: 0, status: this.config.approvalRequired ? 'PENDING_APPROVAL' : 'READY', target: input.target ? target(input.target) : null, protection: { enabled: true, bounds: input.target ? worldBounds(base, target(input.target)) : null }, activeBots: [], progress: { total: base.blocks.length, completed: 0, failed: 0, pending: base.blocks.length }, placements: Object.fromEntries(base.blocks.map(block => [block.key, { status: 'PENDING', attempts: 0, ownerBotId: null, verifiedAt: null, error: null }])), deltas: [], createdAt: now, updatedAt: now }; await this.repository.create(project); await this.record(project.id, 'IMPORTED', { status: project.status }); return this.get(project.id); }
  ...
  async transform(id, input = {}) { const project = await this.get(id); if (!['PENDING_APPROVAL', 'READY'].includes(project.status)) throw new ConflictError(`Blueprint transform requires PENDING_APPROVAL or READY, current ${project.status}`); const rotation = Number(input.rotation ?? 0); if (![0, 90, 180, 270].includes(rotation)) throw new ValidationError('rotation must be 0, 90, 180, or 270'); const mirrorX = Boolean(input.mirrorX); const mirrorZ = Boolean(input.mirrorZ); const origin = project.sourceOrigin ?? project.origin; const sourceBlocks = project.sourceBlocks ?? project.blocks; const blocks = sourceBlocks.map(block => transformBlock(block, origin, rotation, mirrorX, mirrorZ)); const transformed = importBlueprint({ name: project.name, format: project.format, origin, blocks, metadata: { ...project.metadata, transform: { rotation, mirrorX, mirrorZ } } }, this.config); const placements = Object.fromEntries(transformed.blocks.map(block => [block.key, { status: 'PENDING', attempts: 0, ownerBotId: null, verifiedAt: null, error: null }])); await this.repository.update(id, { sourceBlocks, sourceOrigin: origin, origin, blocks: transformed.blocks, bounds: transformed.bounds, materials: transformed.materials, checksum: transformed.checksum, placements, progress: { total: transformed.blocks.length, completed: 0, failed: 0, pending: transformed.blocks.length }, transform: { rotation, mirrorX, mirrorZ }, updatedAt: iso() }); await this.record(id, 'TRANSFORMED', { rotation, mirrorX, mirrorZ }); return this.get(id); }
  async place(id, input = {}) { const project = await this.get(id); if (!['PENDING_APPROVAL', 'READY', 'PAUSED', 'FAILED'].includes(project.status)) throw new ConflictError(`Blueprint placement cannot change while ${project.status}`); const destination = target(input.target); const protection = { enabled: true, bounds: worldBounds(project, destination) }; await this.repository.update(id, { target: destination, protection, updatedAt: iso() }); await this.record(id, 'PLACEMENT_MOVED', { target: destination, bounds: protection.bounds, source: String(input.source ?? 'client') }); await this.syncProtection(); return this.get(id); }
...
function transformBlock(block, origin, rotation, mirrorX, mirrorZ) { let x = block.x - origin.x; let z = block.z - origin.z; if (mirrorX) x = -x; if (mirrorZ) z = -z; const point = rotation === 90 ? { x: -z, z: x } : rotation === 180 ? { x: -x, z: -z } : rotation === 270 ? { x: z, z: -x } : { x, z }; const properties = structuredClone(block.properties ?? {}); if (['north','east','south','west'].includes(properties.facing)) { let facing = properties.facing; if (mirrorX) facing = ({ east: 'west', west: 'east', north: 'north', south: 'south' })[facing]; if (mirrorZ) facing = ({ north: 'south', south: 'north', east: 'east', west: 'west' })[facing]; const index = (['north','east','south','west'].indexOf(facing) + rotation / 90) % 4; properties.facing = ['north','east','south','west'][index]; } return { x: origin.x + point.x, y: block.y, z: origin.z + point.z, name: block.name, properties, blockEntity: block.blockEntity }; }
